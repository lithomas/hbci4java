...
/**
 * <p>
 * Ergebnisse der Abfrage von Kontoumsatzinformationen. Ein Objekt dieser Klasse entspricht einen Kontoauszug. Ein Kontoauszug ist in einzelne Buchungstage
 * unterteilt. Für jeden einzelnen Buchungstag wiederum gibt es eine Anzahl von Umsatzzeilen (das entspricht je einem Eintrag auf dem "normalen" Kontoauszug auf
 * Papier). Jede einzelne Umsatzzeile wiederum enthält die einzelnen Informationen zu genau einer Transaktion.
 * </p>
 * <p>
...
public class GVRKUms extends HBCIJobResultImpl {
    ...
    public static class UmsLine implements Serializable {
        ...
        /**
         * Art der Buchung (bankinterner Code). Nur wenn hier ein Wert ungleich <code>999</code> drinsteht, enthalten die Attribute <code>text</code>,
         * <code>primanota</code>, <code>usage</code>, <code>other</code> und <code>addkey</code> sinnvolle Werte. Andernfalls sind all diese Informationen
         * möglicherweise im Feld <code>additional</code> enthalten, allerdings in einem nicht definierten Format (siehe auch <code>additional</code>).
         */
        public String gvcode;

        /**
         * <p>
         * Zusatzinformationen im Rohformat. Wenn Zusatzinformationen zu dieser Transaktion in einem unbekannten Format vorliegen, dann enthält dieser String
         * diese Daten (u.U. ist dieser String leer, aber nicht <code>null</code>). Das ist genau dann der Fall, wenn der Wert von <code>gvcode</code> gleich
         * <code>999</code> ist.
         * </p>
         * <p>
         * Wenn die Zusatzinformationen aber ausgewertet werden können (und <code>gvcode!=999</code>), so ist dieser String <code>null</code>, und die Felder
         * <code>text</code>, <code>primanota</code>, <code>usage</code>, <code>other</code> und <code>addkey</code> enthalten die entsprechenden Werte (siehe
         * auch <code>gvcode</code>)
         * </p>
         */
        public String additional;

        /**
         * Beschreibung der Art der Buchung (optional). Nur wenn <code>gvcode!=999</code>! (siehe auch <code>additional</code> und <code>gvcode</code>)
         */
        public String text;
        /**
         * Primanotakennzeichen (optional). Nur wenn <code>gvcode!=999</code>! (siehe auch <code>additional</code> und <code>gvcode</code>)
         */
        public String primanota;
        /**
         * Liste von Strings mit den Verwendungszweckzeilen. Nur wenn <code>gvcode!=999</code>! (siehe auch <code>additional</code> und <code>gvcode</code>)
         */
        public List<String> usage;
        /**
         * Gegenkonto der Buchung (optional). Nur wenn <code>gvcode!=999</code>! (siehe auch <code>additional</code> und <code>gvcode</code>)
         */
        public Konto other;
        /**
         * Erweiterte Informationen zur Art der Buchung (bankintern, optional). Nur wenn <code>gvcode!=999</code>! (siehe auch <code>additional</code> und
         * <code>gvcode</code>)
         */
    ...
    /**
     * Enthält alle Transaktionen eines einzelnen Buchungstages. Dazu gehören das Datum des jeweiligen Tages, der Anfangs- und Endsaldo sowie die Menge aller
     * dazugehörigen Umsatzeilen
     */
    public static class BTag implements Serializable {
        /**
         * <p>
         * Konto, auf das sich die Umsatzdaten beziehen (Kundenkonto). Einige Kreditinstitute geben fehlerhafte Kontoauszüge zurück, was zur Folge haben kann,
         * dass dieses Feld nicht richtig belegt ist. Tritt ein solcher Fall ein, so kann es vorkommen, dass von dem <code>Konto</code>-Objekt nur das Feld
         * <code>number</code> gefüllt ist, und zwar mit den Informationen, die das Kreditinstitut zur Identifizierung dieses Kontos zurückgibt.
         * </p>
         * <p>
         * Normalerweise bestehen diese Informationen aus BLZ und Kontonummer, die dann auch korrekt in das <code>Konto</code>-Objekt eingetragen werden. Liegen
         * diese Informationen aber gar nicht oder in einem falschen bzw. unbekannten Format vor, so werden diese Daten komplett in das <code>number</code>-Feld
         * geschrieben.
         * </p>
         */
        public Konto my;
    ...
    /**
     * Dieses Feld enthält einen String, der den nicht-auswertbaren Teil der Kontoauszüge enthält. Es dient nur zu Debugging-Zwecken und sollte eigentlich immer
     * <code>null</code> bzw. einen leeren String enthalten. Wenn das nicht der Fall ist, dann konnten die empfangenen Kontoauszüge nicht richtig geparst
     * werden, und dieser String enthält den "Schwanz" der Kontoauszugsdaten, bei dem das Parsing-Problem aufgetreten ist.
     */
    ...
    /**
     * Gibt alle Transaktionsdatensätze in einer "flachen" Struktur zurück. D.h. nicht in einzelne Buchungstage unterteilt, sondern in einer Liste analog zu
     * einem "normalen" Kontoauszug.
     * 
    ...
    private void verifyMT94xParsing(String where) {
        ...
        if (restMT940 != null && restMT940.length() != 0) {
            HBCIUtils.log(where + ": mt940 has not been parsed successfully " + "- probably returned data will be incomplete. "
                            + "check variable 'restMT940' (or set logging level to 4 (=DEBUG)) " + "to see the data that could not be parsed.",
                            HBCIUtils.LOG_WARN);
            HBCIUtils.log("restMT940: " + restMT940, HBCIUtils.LOG_DEBUG);
        }

        if (restMT942 != null && restMT942.length() != 0) {
            HBCIUtils.log(where + ": mt942 has not been parsed successfully " + "- probably returned data will be incomplete. "
                            + "check variable 'restMT942' (or set logging level to 4 (=DEBUG)) " + "to see the data that could not be parsed.",
                            HBCIUtils.LOG_WARN);
        ...
        try {
            ...
            while (buffer.length() != 0) {
                ...
                while (true) {
                    ...
                    line.value.setValue(HBCIUtilsInternal.string2Long((cd.equals(negValueIndikator) ? "-" : "")
                                    + st_ums.substring(next, npos).replace(',', '.'), 100));
                    next = npos + 4;
                ...
                if (st_end != null) {
                    ...
                    btag.end.value = new Value((cd.equals("D") ? "-" : "") + st_end.substring(10).replaceAll("\\s", "").replace(',', '.'), st_end.substring(7,
                                    10));
                }
