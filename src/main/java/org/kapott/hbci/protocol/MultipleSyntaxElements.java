...
public abstract class MultipleSyntaxElements {
    ...
    /**
     * erzeugt einen neuen eintrag in der elements liste; dabei wird ein syntaxelement erzeugt, das im xml-node ref referenziert wird; idx ist die indexnummer
     * des zu erzeugenden syntaxelementes innerhalb der elementlist
     */
    ...
    protected abstract SyntaxElement parseAndAppendNewElement(Node ref, String path, char predelim, int idx, StringBuffer res, int fullResLen, Document syntax,
    ...
    private void initData(Node ref, String path, Document syntax) {
        ...
        try {
            ...
            /*
             * erzeugen sovieler syntaxelemente, bis die mindestanzahl aus der syntaxdefinition erreicht ist
             */
    ...
    protected boolean propagateValue(String destPath, String value, boolean tryToCreate, boolean allowOverwrite) {
        ...
        if (tryToCreate) {
            ...
            if (destPath.startsWith(path) && destPathLen > pathLen && destPath.charAt(pathLen) == '_') {
    ...
    private void initData(Node ref, String path, char predelim0, char predelim1, StringBuffer res, int fullResLen, Document syntax,
                    Hashtable<String, String> predefs, Hashtable<String, String> valids) {
        this.ref = null;
        this.syntax = null;
        this.syntaxIdx = -1;
        ...
        try {
            while (!ready) {
                ...
                try {
                    ...
                    SyntaxElement child = parseAndAppendNewElement(ref, path, (idx == 0) ? predelim0 : predelim1, idx, res, fullResLen, syntax, predefs, valids);
                    if (child != null)
                        child.setParent(this);
                    ...
                    /*
                     * wenn bisher weniger als die mindestanzahl geklappt hat, dann exception werfen
                     */
                    if (idx < minnum)
                ...
                /*
                 * wenn die maxanzahl erreicht wurde oder wenn die maxanzahl nicht definiert ist, aber kein neues element erzeugt werden konnte (wenn
                 * gesicherter reststring und tatsaechlicher reststring gleich sind; minnum ist aber erreicht), dann diesen container normal beenden
                 */
                if ((maxnum != 0 && idx >= maxnum) || (maxnum == 0 && save.toString().equals(res.toString()) && !emptyElementFound)) {
                    ready = true;
                }
            }
        } catch (RuntimeException e) {
    ...
    /**
     * anlegen einer neuen syntaxelementlist beim parsen des strings res; - ref ist dabei die referenz auf einen xml-node, der das syntaxelement festlegt, fuer
     * den die syntaxelementlist erzeugt werden soll; - predefs siehe SyntaxElement() - predelim0 ist der delimiter, der vor dem ersten element innerhalb dieser
     * syntaxelementlist auftreten muesste; - predelim1 ist der delimiter, der vor dem zweiten, dritten, usw. element in der syntaxelementlist auftreten muesste
     * (der unterschied zwischen predelim0 und predelim1 ist der, dass predelim0 evtl. von uebergeordneten elementen propagiert wird (z.b. wenn die
     * syntaxelementlist selbst das erste syntaxelement einer msg repraesentiert), predelim1 ist allerdings immer der delimiter, der fuer das aktuell
     * uebergeordnete syntaxelement zu verwenden ist)
     */
    protected MultipleSyntaxElements(Node ref, String path, char predelim0, char predelim1, StringBuffer res, int fullResLen, Document syntax,
                    Hashtable<String, String> predefs, Hashtable<String, String> valids) {
        initData(ref, path, predelim0, predelim1, res, fullResLen, syntax, predefs, valids);
    }

    protected void init(Node ref, String path, char predelim0, char predelim1, StringBuffer res, int fullResLen, Document syntax,
                    Hashtable<String, String> predefs, Hashtable<String, String> valids) {
        initData(ref, path, predelim0, predelim1, res, fullResLen, syntax, predefs, valids);
    }
