...
public final class SF extends SyntaxElement {
    ...
    protected MultipleSyntaxElements createAndAppendNewChildContainer(Node ref, Document syntax) {
        ...
        if (((Element) ref).getAttribute("minnum").equals("0")) {
            HBCIUtils.log("will not create container " + getPath() + " -> " + ((Element) ref).getAttribute("type") + " with minnum=0", HBCIUtils.LOG_INTERN);
        } else {
    ...
    public SF(String type, String name, String path, char predelim, int idx, StringBuffer res, int fullResLen, Document syntax,
    ...
    public void init(String type, String name, String path, char predelim, int idx, StringBuffer res, int fullResLen, Document syntax,
                    Hashtable<String, String> predefs, Hashtable<String, String> valids) {
    ...
    protected MultipleSyntaxElements parseNewChildContainer(Node segref, char predelim0, char predelim1, StringBuffer res, int fullResLen, Document syntax,
                    Hashtable<String, String> predefs, Hashtable<String, String> valids) {
        MultipleSyntaxElements ret = null;
    ...
    protected MultipleSyntaxElements parseAndAppendNewChildContainer(Node segref, char predelim0, char predelim1, StringBuffer res, int fullResLen,
        ...
        if ((segref.getNodeName()).equals("SEG")) {
            ...
            if (segRefId[0].equals(nextSegId[0]) && segRefId[1].equals(nextSegId[1]) || segRefId[0].equals("") || segRefId[1].equals("")) {
                ...
                /*
                 * this is a very ugly hack for the ugly parser code: in certain cases it may happen that hbci4java takes a HIUPA segment as a
                 * BPD-Params-Template segment. the following code tries to avoid this, but the solution is not "general". TODO: we really should replace the
                 * ugly message engine soon!
                 */

                boolean parseNext = true;
                if (getName().startsWith("Params")) {
                    /*
                     * we are in the BPD-Params SF. only child SEGs with a segcode of the form ".....S" are allowed here
                     */
