...
public abstract class SyntaxElement {
    ...
    /**
     * < @internal @brief indicates if this element is really valid, i.e. will appear in an outgoing hbci message resp. in returned results from an incoming
     * message
     */
    ...
    /**
     * beim parsen: haengt an die 'childElements' ein neues Element an. der xml-knoten 'ref' gibt an, um welches element es sich dabei handelt; aus 'res' (der
     * zu parsende String) wird der wert fuer das element ermittelt (falls es sich um ein de handelt); in 'predefined' ist der wert des elementes zu finden, der
     * laut syntaxdefinition ('syntax') an dieser stelle auftauchen mueste (optional; z.b. fuer segmentcodes); 'predelim*' geben die delimiter an, die direkt
     * vor dem zu erzeugenden syntaxelement auftauchen muessten
     */
    protected abstract MultipleSyntaxElements parseNewChildContainer(Node ref, char predelim0, char predelim1, StringBuffer res, int fullResLen,
                    Document syntax, Hashtable<String, String> predefs, Hashtable<String, String> valids);

    /**
     * wird fuer datenelemente benoetigt, die sonst unbeabsichtigt generiert werden koennten. das problem ist, dass es datenelemente (bisher nur bei segmenten
     * bekannt) gibt, die aus einigen "required" unterelementen bestehen und aus einigen optionalen unterelementen. wenn *alle* "required" elemente bereits
     * durch predefined values bzw. durch automatisch generierte werte vorgegeben sind, dann wird das entsprechende element erzeugt, da es auch ohne angabe der
     * optionalen unterelemente gueltig ist.
     * 
     * es ist aber u.U. gar nicht beabsichtigt, dass dieses element erzeugt wird (beispiel segment "KIOffer", wo nur die DEG SegHead required ist, alle anderen
     * elemente sind optional). es kann also vorkommen, dass ein element *unbeabsichtigt* nur aus den vorgabedaten erzeugt wird.
     * 
     * bei den elementen, bei denen das passieren kann, wird in der xml-spezifikation deshalb zusaetzlich das attribut "needsRequestTag" angegeben. der wert
     * dieses attributes wird hier in der variablen @p needsRequestTag gespeichert.
     * 
     * beim ueberpruefen, ob das aktuelle element gueltig ist (mittels @c validate() ), wird neben der gueltigkeit aller unterelemente zusaetzlich ueberprueft,
     * ob dieses element ein request-tag benoetigt, und wenn ja, ob es vorhanden ist. wenn die
     * 
     * @p needsRequestTag -bedingung nicht erfuellt ist, ist auch das element ungueltig, und es wird nicht erzeugt.
     * 
     *    das vorhandensein eines request-tags wird in der variablen @haveRequestTag gespeichert. dieses flag kann fuer ein bestimmtes element gesetzt werden,
     *    indem ihm der wert "requested" zugewiesen wird. normalerweise kann nur DE-elementen ein wert zugewiesen werden, diese benoetigen aber kein
     *    request-tag. wird also einem gruppierenden element der wert "requested" zugewiesen, dann wird das durch die methode @c propagateValue() als explizites
     *    setzen des @p haveRequestTag interpretiert.
     * 
     *    alle klassen und methoden, die also daten fuer die erzeugung von nachrichten generieren, muessen u.U. fuer bestimmte syntaxelemente diesen
     *    "requested"-wert setzen.
     * 
     *    needsRequestTag kann komplett weg, oder? -- nein. Für GV-Segmente gilt das schon. Die Überprüfung des requested-Werted findet aber in der
     *    *allgemeinen* SyntaxElement-Klasse statt, wo auch andere Segmente (z.b. MsgHead) erzeugt werden. Wenn als "allgemeiner" Check der Check
     *    "if SEG.isRequested" eingeführt werden würde, dann würde der nur bei tatsächlich gewünschten GV-Segmenten true ergeben. Bei MsgHead-Segmenten z.B.
     *    würde er false ergeben (weil diese Segmente niemals auf "requested" gesetzt werden). Deshalb darf diese "requested"-Überprüfung nur bei den
     *    Syntaxelementen stattfinden, bei denen das explizit gewünscht ist (needsRequestTag).
     */
    ...
    private void initData(String type, String name, String ppath, int idx, Document syntax) {
        ...
        /*
         * der pfad wird gebildet aus bisherigem pfad plus name des elementes plus indexnummer, falls diese groesser 0 ist
         */
        ...
        if (syntax != null) {
            ...
            try {
                ...
                /*
                 * durchlaufen aller "value"-knoten und setzen der werte der entsprechenden de
                 */
    ...
    /**
     * es wird ein syntaxelement mit der id 'name' initialisiert; der pfad bis zu diesem element wird in 'path' uebergeben; 'idx' ist die nummer dieses
     * elementes innerhalb der syntaxelementliste fuer dieses element (falls ein bestimmtes syntaxelement mehr als einmal auftreten kann)
     */
    ...
    /**
     * loop through all child-elements; the segments found there will be sequentially enumerated starting with num startValue; if startValue is zero, the
     * segments will not be enumerated, but all given the number 0
     * 
     * @param startValue
     *            value to be used for the first segment found
    ...
    private void initData(String type, String name, String ppath, char predelim, int idx, StringBuffer res, int fullResLen, Document syntax,
                    Hashtable<String, String> predefs, Hashtable<String, String> valids) {
        this.type = type;
        this.name = name;
        this.parent = null;
        this.childContainers = new ArrayList<MultipleSyntaxElements>();
        ...
        this.def = null;
        /*
         * position des aktuellen datenelementes berechnet sich aus der gesamtlänge des ursprünglichen msg-strings minus der länge des reststrings, der jetzt zu
         * parsen ist, und der mit dem aktuellen datenelement beginnt
         */
        ...
        if (syntax != null) {
            ...
            /*
             * fuellen der 'predefs'-tabelle mit den in der syntaxbeschreibung vorgegebenen werten
             */
            NodeList valueNodes = ((Element) def).getElementsByTagName("value");
            ...
            try {
                ...
                for (Node ref = def.getFirstChild(); ref != null; ref = ref.getNextSibling()) {
                    if (ref.getNodeType() == Node.ELEMENT_NODE) {
                        MultipleSyntaxElements child = parseAndAppendNewChildContainer(ref, ((counter++) == 0) ? predelim : getInDelim(), getInDelim(), res,
                                        fullResLen, syntax, predefs, valids);
                        ...
                        if (child != null) {
                            ...
                            if ((this instanceof SF) && (getName().equals("Params") || getName().equals("GVRes")) && ((MultipleSEGs) child).hasValidChilds()) {
                                break;
                            }
                        }
                    }
    ...
    /**
     * beim parsen: initialisiert ein neues syntaxelement mit der id 'name'; in 'path' wird der pfad bis zu dieser stelle uebergeben 'predelim' gibt das
     * delimiter-zeichen an, das beim parsen vor diesem syntax- element stehen muesste 'idx' ist die nummer des syntaxelementes innerhalb der uebergeordneten
     * liste (die liste repraesentiert das evtl. mehrmalige auftreten eines syntaxelementes, siehe class syntaxelementlist) 'res' ist der zu parsende String
     * 'predefs' soll eine menge von pfad-wert-paaren enthalten, die fuer einige syntaxelemente den wert angeben, den diese elemente zwingend haben muessen
     * (z.b. ein bestimmter segmentcode o.ae.)
     */
    protected SyntaxElement(String type, String name, String path, char predelim, int idx, StringBuffer res, int fullResLen, Document syntax,
                    Hashtable<String, String> predefs, Hashtable<String, String> valids) {
        initData(type, name, path, predelim, idx, res, fullResLen, syntax, predefs, valids);
    }

    protected void init(String type, String name, String path, char predelim, int idx, StringBuffer res, int fullResLen, Document syntax,
                    Hashtable<String, String> predefs, Hashtable<String, String> valids) {
        initData(type, name, path, predelim, idx, res, fullResLen, syntax, predefs, valids);
    }

    protected MultipleSyntaxElements parseAndAppendNewChildContainer(Node ref, char predelim0, char predelim1, StringBuffer res, int fullResLen,
                    Document syntax, Hashtable<String, String> predefs, Hashtable<String, String> valids) {
    ...
    /**
     * fuellt die hashtable 'values' mit den werten der de-syntaxelemente; dazu wird in allen anderen typen von syntaxelementen die liste der child-elemente
     * durchlaufen und deren 'fillValues' methode aufgerufen
     */
    ...
    /**
     * @return the ArrayList containing all child-elements (the elements of the ArrayList are instances of the SyntaxElementArray class
     */
    ...
    /**
     * setzt den wert eines de; in allen syntaxelementen ausser DE wird dazu die liste der child-elemente durchlaufen; jedem dieser child-elemente wird der wert
     * zum setzen uebergeben; genau _eines_ dieser elemente wird sich dafuer zustaendig fuehlen (das DE mit 'path'='destPath') und den wert uebernehmen
     */
    // TODO: code splitten
                ...
                if (!ret && tryToCreate) {
                    ...
                    if (!found) {
                        ...
                        if (found) {
                            ...
                            HBCIUtils.log("  inserting child container with syntaxIdx " + newChildIdx + " at position " + newPosi, HBCIUtils.LOG_INTERN);
                        ...
                        HBCIUtils.log("  subtype " + subType + " already existing - will not try to create", HBCIUtils.LOG_INTERN);
                    }
                }
            }
        }
    ...
    /**
     * @return den wert eines bestimmten DE; funktioniert analog zu 'propagateValue'
     */
    ...
    /**
     * @param path
     *            path to the element to be returned
    ...
    /**
     * @param type
     *            the name of the syntaxelement to be returned
     * @param syntax
     *            the structure containing the current syntaxdefinition
    ...
    /**
     * diese toString() methode wird benutzt, um den wert eines de-syntaxelementes in human-readable-form zurueckzugeben. innerhalb eines de-elementes wird der
     * wert in der hbci-form gespeichert
     */
    ...
    /**
     * ueberpreuft, ob das syntaxelement alle restriktionen einhaelt; ist das nicht der fall, so wird eine Exception ausgeloest. die meisten syntaxelemente
     * koennen sich nicht selbst ueberpruefen, sondern rufen statt dessen die validate-funktion der child-elemente auf
     */
    public void validate() {
        if (!needsRequestTag || haveRequestTag) {
            ...
            /*
             * wenn keine exception geworfen wurde, dann ist das aktuelle element offensichtlich valid
             */
            setValid(true);
        }
    }
