...
/**
 * <p>
 * Hilfsklasse zum Erzeugen von DTAUS-Datensätzen für die Verwendung in Sammelüberweisungen und Sammellastschriften. Diese Klasse kann verwendet werden, um den
 * DTAUS-Datenstrom zu erzeugen, der für Sammellastschriften und -überweisungen als Job-Parameter angegeben werden muss.
 * </p>
 * <p>
 * In einem DTAUS-Objekt werden ein oder mehrere Transaktionen gespeichert. Dabei müssen alle Transaktionen entweder Lastschriften oder Überweisungen sein.
 * Außerdem wird für alle Transaktionen das gleiche "Auftraggeberkonto" angenommen (bei Überweisungen also das Belastungskonto, bei Lastschriften das Konto, auf
 * das der Betrag gutgeschrieben wird).
 * </p>
 * <p>
 * In der Regel wird zunächst ein <code>DTAUS</code>-Objekt erzeugt. Dazu wird der Konstruktor {@link #DTAUS(Konto,int)} verwendet, womit gleichzeit das zu
 * verwendende Auftraggeberkonto und der Typ des Sammelauftrages (<code>TYPE_CREDIT</code> für Sammelüberweisungen, <code>TYPE_DEBIT</code> für
 * Sammellastschriften) festgelegt wird. Anschließend können beliebig viele {@link DTAUS.Transaction}-Objekte erzeugt werden, welche jeweils eine Transaktion
 * darstellen. Jedes so erzeugte Objekt kann mit {@link #addEntry(DTAUS.Transaction)} zum Sammelauftrag hinzugefügt werden. Die Methode {@link #toString()}
...
public class DTAUS {
    /**
     * Daten einer einzelnen Transaktion, die in einen Sammelauftrag übernommen werden soll. Vor dem Hinzufügen dieser Transaktion zum Sammelauftrag müssen alle
     * Felder dieses Transaktions-Objektes mit den jeweiligen Auftragsdaten gefüllt werden.
     */
    public class Transaction {
        /**
         * <p>
         * Konto des Zahlungsempfängers bzw. des Zahlungspflichtigen. Soll dieser Einzelauftrag in eine Sammelüberweisung eingestellt werden, so muss in diesem
         * Feld die Kontoverbindung des Zahlungsempfängers eingestellt werden. Bei Sammellastschriften ist hier die Kontoverbindung des Zahlungspflichtigen
         * einzustellen.
         * </p>
         * <p>
         * Von dem verwendeten {@link Konto}-Objekt müssen mindestens die Felder <code>blz</code>, <code>number</code> und <code>name</code> richtig belegt
         * sein.
         * </p>
         */
        ...
        /**
         * Textschlüssel für den Auftrag. Bei Sammelüberweisungen ist dieses Feld mit '51' vorbelegt, bei Sammellastschriften mit '05'. Dieser Wert kann
         * überschrieben werden, gültige Werte finden sich in den Job-Restrictions (siehe {@link org.kapott.hbci.GV.HBCIJob#getJobRestrictions()}).
         */
        public String key;

        /**
         * Zusätzlicher Textschlüssel (wird i.d.R. bankintern verwendet). Dieser Wert muss aus drei Ziffern bestehen und ist mit '000' vorbelegt. Das manuelle
         * Setzen dieses Wertes ist in den meisten Fällen nicht nötig (außer für Leute, die wissen was sie tun ;-) ).
         */
    ...
    /**
     * Erzeugen eines neuen Objektes für die Aufnahme von Sammelaufträgen. <code>myAccount</code> ist dabei das "eigene" Konto, welches bei Sammelüberweisungen
     * als Belastungskonto und bei Sammellastschriften als Gutschriftkonto verwendet wird. Von dem {@link Konto}-Objekt müssen mindestens die Felder
     * <code>blz</code>, <code>number</code>, <code>curr</code> und <code>name</code> richtig gesetzt sein. <br/>
     * <code>execdate</code> gibt das Datum an, wann dieser Sammelauftrag ausgeführt werden soll. ACHTUNG: <code>execdate</code> wird zur Zeit noch nicht
     * ausgewertet!
     * 
     * @param myAccount
     *            Gegenkonto für die enthaltenen Aufträge
     * @param type
     *            <ul>
     *            <li><code>TYPE_CREDIT</code> für Sammelüberweisungen,</li>
     *            <li><code>TYPE_DEBIT</code> für Sammellastschriften</li>
     *            </ul>
     * @param execdate
     *            Ausführungsdatum für diesen Sammelauftrag; <code>null</code>, wenn kein Ausführungsdatum gesetzt werden soll (sofortige Ausführung)
     */
    ...
    /**
     * Hinzufügen eines einzelnen Auftrages zu diesem Sammelauftrag. Das {@link DTAUS.Transaction}-Objekt, welches hier als Argument benötigt wird, muss mit '
     * <code>dtaus.new&nbsp;Transaction()</code>' erzeugt werden ('<code>dtaus</code>' ist dabei das aktuelle <code>DTAUS</code>-Objekt).
     * 
     * @param entry
    ...
    /**
     * Rückgabe des Sammelauftrages im DTAUS-Format. Der Rückgabewert dieser Methode kann direkt als Parameterwert für den Parameter '<code>data</code>' bei
     * Sammelaufträgen verwendet werden (für eine Parameterbeschreibung siehe Paketbeschreibung des Paketes <code>org.kapott.hbci.GV</code>).
     * 
    ...
    public String toString() {
        ...
        ret.append(expand(Long.toString(curr == CURR_DM ? sumDM : 0), 13, (byte) 0x30, ALIGN_RIGHT));
        ...
        ret.append(expand(Long.toString(curr == CURR_EUR ? sumEUR : 0), 13, (byte) 0x30, ALIGN_RIGHT));
