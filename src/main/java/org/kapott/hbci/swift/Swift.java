        ...
        while (true) {
            ...
            if (startpos != -1) {
                ...
                if (matcher.find(startpos)) {
                    ...
                    /*
                     * no next tag found - use end of stream without the trailing \r\n-
                     */
                    endpos = st.length() - 3;
                }
        ...
        if (pos != -1) {
            ...
            while (true) {
                ...
                if (endpos != -1 && endpos + 2 < st.length()) {
                    /*
                     * "?" is far enough before end to make it possible to be followed by two digits
                     */

                    if (st.charAt(endpos + 1) >= '0' && st.charAt(endpos + 1) <= '9' && st.charAt(endpos + 2) >= '0' && st.charAt(endpos + 2) <= '9') {
                        /*
                         * the "?" must be followed by two digits, a single "?" does NOT mark the end of value
                         */
                        break;
                    }
                } else {
                    /*
                     * the "?" is near the end of the string, so we break out here und use the complete string as value
                     */
                    endpos = -1;
                    break;
                }
