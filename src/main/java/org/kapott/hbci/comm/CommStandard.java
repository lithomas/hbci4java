...
public final class CommStandard extends Comm {
    ...
    public CommStandard(HBCIPassportInternal parentPassport) {
        ...
        HBCIUtils.log("opening connection to " + parentPassport.getHost() + ":" + parentPassport.getPort().toString(), HBCIUtils.LOG_DEBUG);
        ...
        try {
            ...
            if (socksServer != null && socksServer.trim().length() != 0) {
                ...
                HBCIUtils.log("using SOCKS server at " + socksHost + ":" + socksPort, HBCIUtils.LOG_DEBUG);
            ...
            s.connect(new InetSocketAddress(parentPassport.getHost(), parentPassport.getPort().intValue()));
            i = s.getInputStream();
            o = s.getOutputStream();
        } catch (Exception e) {
