...
public final class CommPinTan extends Comm {
    ...
    public CommPinTan(HBCIPassportInternal parentPassport) {
        ...
        try {
            ...
            HBCIUtils.log("creating a connection to https://" + host + ":" + parentPassport.getPort() + path + " and checking the certificate",
                            HBCIUtils.LOG_INFO);
            ...
            if (proxyData.length == 2) {
                HBCIUtils.log("HTTPS connections will be made using proxy " + proxyData[0] + "(Port " + proxyData[1] + ")", HBCIUtils.LOG_INFO);
    ...
    protected void ping(MSG msg) {
        try {
            ...
            if (!checkCert || debugging) {
                ...
                HBCIUtils.log("activating modified socket factory for" + " checkCert=" + checkCert + " and debugging=" + debugging, HBCIUtils.LOG_DEBUG);
                connSSL.setSSLSocketFactory(this.mySocketFactory);

                if (!checkCert) {
                    // checkcert=0 --> use dummy hostname verifier that always succeeds
                    HBCIUtils.log("activating modified hostname verifier because cert checking is disabled", HBCIUtils.LOG_DEBUG);
