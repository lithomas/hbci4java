...
/**
 * <p>
 * Wrapper-Klasse, die bei Verwendung des threaded-callback-Mechanismus benötigt wird. Sollen Callbacks synchron behandelt werden, so ist es zur Aktivierung
 * dieses Mechanismus' notwendig, das "normale" Callback-Objekt in einer Instanz dieser Klasse zu kapseln. Diese Klasse sorgt dafür, dass "normale" (asynchrone)
 * Callbacks wie gewohnt von dem "normalen" Callback-Objekt behandelt werden. Bei synchron zu behandelnden Callbacks sorgt diese Callback-Implementierung dafür,
 * dass {@link org.kapott.hbci.manager.HBCIHandler#executeThreaded() hbci.executeThreaded()} terminiert.
 * </p>
 * <p>
 * Mehr Informationen sind in der Datei <code>README.ThreadedCallbacks</code> sowie unter
 * {@link HBCICallback#useThreadedCallback(HBCIPassport, int, String, int, StringBuffer)} zu finden.
 * </p>
 */
public final class HBCICallbackThreaded extends AbstractHBCICallback {
    ...
    /**
     * Erzeugt eine Instanz dieser Klasse. Ein HBCIThreadedCallback-Objekt muss bei {@link org.kapott.hbci.manager.HBCIUtils#init(Properties, HBCICallback)} als
     * Callback-Objekt übergeben werden, wenn der threaded-callback-Mechanismus benutzt werden soll.
     * 
     * @param realCallback
     *            eine Instanz einer "normalen" Callback-Klasse. Alle asynchron zu behandelnden Callbacks (der Normalfall) werden an dieses Objekt weitergegeben
    ...
    /**
     * Für asynchron zu behandelnde Callbacks wird der Aufruf an das "normale" Callback-Objekt weitergereicht. Synchron zu behandelnde Callbacks werden von
     * dieser Methode behandelt, in dem der entsprechende Aufruf von {@link org.kapott.hbci.manager.HBCIHandler#executeThreaded()} terminiert und
     * Callback-Info-Daten zurückgibt.
     */
    public void callback(HBCIPassport passport, int reason, String msg, int datatype, StringBuffer retData) {
        ...
        if (sync_main == null) {
            ...
            if (realCallback.useThreadedCallback(passport, reason, msg, datatype, retData)) {
                ...
                sync_hbci.startWaiting(Integer.parseInt(HBCIUtils.getParam("kernel.threaded.maxwaittime", "300")),
                                "no callback data received from main thread - timeout");
