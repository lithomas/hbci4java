...
/**
 * Callback-Klasse für Ein-/Ausgabe über IO-Streams. Dabei handelt es sich eine Callback-Klasse, die Ausgaben auf einem PrintStream ausgibt und Eingaben über
 * einen BufferedReader liest. Die Klasse {@link org.kapott.hbci.callback.HBCICallbackConsole HBCICallbackConsole} ist eine abgeleitete Klasse, welche STDOUT und
 * STDIN für die beiden I/O-Streams verwendet.
 */
public class HBCICallbackIOStreams extends AbstractHBCICallback {
    ...
    /**
     * Schreiben von Logging-Ausgaben in einen <code>PrintStream</code>. Diese Methode implementiert die Logging-Schnittstelle des
     * {@link org.kapott.hbci.callback.HBCICallback}-Interfaces</a>. Die Log-Informationen, die dieser Methode übergeben werden, werden formatiert auf dem
     * jeweiligen <code>outStream</code> ausgegeben. In dem ausgegebenen String sind in enthalten das Log-Level der Message, ein Zeitstempel im Format "
     * <code>yyyy.MM.dd HH:mm:ss.SSS</code>", die Namen der ThreadGroup und des Threads, aus dem heraus die Log-Message erzeugt wurde, der Klassenname der
     * Klasse, welche die Log-Ausgabe erzeugt hat sowie die eigentliche Log-Message
     */
    ...
    /**
     * Diese Methode reagiert auf alle möglichen Callback-Ursachen. Bei Callbacks, die nur Informationen an den Anwender übergeben sollen, werden diese auf dem
     * <code>outStream</code> ausgegeben. Bei Callbacks, die Aktionen vom Anwender erwarten (Einlegen der Chipkarte), wird eine entsprechende Aufforderung
     * ausgegeben. Bei Callbacks, die eine Eingabe vom Nutzer erwarten, wird die entsprechende Eingabeaufforderung ausgegeben und die Eingabe vom
     * <code>inStream</code> gelesen.
     */
            ...
            case NEED_SIZENTRY_SELECT:
                ...
                while (tok.hasMoreTokens()) {
                    ...
                    getOutStream().println(
                                    tok2.nextToken() + ": " + HBCIUtilsInternal.getLocMsg("BLZ") + "=" + (tempblz = tok2.nextToken()) + " ("
                                                    + HBCIUtils.getNameForBLZ(tempblz) + ") " + HBCIUtilsInternal.getLocMsg("USERID") + "=" + tok2.nextToken());
                }
    ...
    /**
     * Wird diese Methode von <em>HBCI4Java</em> aufgerufen, so wird der aktuelle Bearbeitungsschritt (mit evtl. vorhandenen zusätzlichen Informationen) auf
     * <code>outStream</code> ausgegeben.
     */
