...
/**
 * Basisklasse für die beiden <code>ShowLowlevelGV*</code>-Tools. Diese Klasse wird nur intern verwendet.
 */
        ...
        if (ref.getAttribute("type").length() != 0) {
            ...
            if (ref.getNodeName().equals("DE")) {
                if (currentLevel >= minLevel) {
                    ...
                    System.out.println(spaces + pathWithDot(path) + name + ":" + datatype + ((minlen.length() != 0) ? (" min:" + minlen) : "")
                                    + ((maxlen.length() != 0) ? (" max:" + maxlen) : "") + " {" + minnum + "," + maxnum + "}");
                }
            } else {
                ...
                for (int i = 0; i < len; i++) {
                    ...
                    if (content.getNodeType() == Node.ELEMENT_NODE)
                        displayContentRef(nextPath, (Element) content, syntax, nextIndent, currentLevel + 1, minLevel);
                }
            }
        }
    }
