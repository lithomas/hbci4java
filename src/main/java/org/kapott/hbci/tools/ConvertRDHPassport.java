...
/**
 * <p>
 * Tool zum Konvertieren von "alten" RDH-Passport-Dateien in "neue" RDHNew-Dateien. Die Passport-Variante "<code>RDH</code>" sollte ab sofort nicht mehr benutzt
 * werden, sondern statt dessen die Variante "<code>RDHNew</code>". Siehe dazu auch die Datei <code>README.RDHNew</code> im <em>HBCI4Java</em>-Archiv.
 * </p>
 * <p>
 * Die Konvertierung von RDH-Passports in RDHNew-Passports kann auch mit dem separat verfügbaren HBCI4Java Passport Editor durchgeführt werden. Mit diesem
 * Editor können darüber hinaus RDHNew-Passports auch wieder in RDH-Passports konvertiert werden.
 * </p>
 * <p>
 * Bei der <code>RDHNew</code>-Variante hat sich im Vergleich zu <code>RDH</code> das Dateiformat der Schlüsseldateien geändert. Deshalb muss vor Verwendung
 * einer alten <code>RDH</code>-Schlüsseldatei diese erst in das neue Dateiformat konvertiert werden. Das geschieht mit diesem Tool. Vor dessen Anwendung sollte
 * sicherheithalber ein Backup der aktuellen (alten) RDH-Schlüsseldatei angelegt werden (das sollte aber sowieso vorhanden sein!).
 * </p>
 * <p>
 * Aufgerufen wird der Konverter mit
 * 
 * <pre>
 * java -cp ... org.kapott.hbci.tools.ConvertRDHPassport
 * </pre>
 * 
 * Es handelt sich um ein interaktives Programm. Nach dem Start wird nach dem Dateinamen einer existierenden RDH-Passport-Datei sowie nach dem Passwort für
 * deren Entschlüsselung gefragt. Anschließend wird nach einem <em>neuen(!)</em> Dateinamen für die zu erstellende RDHNew-Passport-Datei sowie nach einem
 * Passwort für deren Verschlüsselung gefragt. Nach Beendigung des Programmes existiert die RDHNew-Passport-Datei, welche ab sofort benutzt werden kann.
 * </p>
 * <p>
 * Um eine RDHNew-Passport-Datei zu benutzen, müssen alle HBCI-Parameter, die vorher mit "<code>client.passport.RDH.*</code>" gesetzt wurden, jetzt mit "
 * <code>client.passport.RDHNew.*</code>" gesetzt werden, und als "<code>client.passport.default</code>" bzw. als Argument zu
 * {@link org.kapott.hbci.passport.AbstractHBCIPassport#getInstance(String)} muss "<code>RDHNew</code>" angegeben werden.
 * </p>
 * <p>
 * Die alte Passport-Datei sollte ab sofort nicht mehr verwendet werden.
 * </p>
 */
public class ConvertRDHPassport {
    ...
    private static String readParam(String[] args, int idx, String st) throws IOException {
        String ret;
