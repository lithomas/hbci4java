...
/**
 * <p>
 * Tool zum Initialisieren und Testen eines HBCI-Passports. Dieses Tool dient einerseits als Vorlage für die Benutzung von <em>HBCI4Java</em> in eigenen
 * Anwendungen und gleichzeitig als Tool, um ein HBCI-Passport einzurichten und zu initialisieren.
 * </p>
 * <p>
 * Für das Einrichten und Initialisieren eines HBCI-Passports gibt es keine speziellen Funktionen, statt dessen wird ein Passport einfach so benutzt, als ob es
 * schon vorhanden wäre. Alle fehlenden Daten holt sich <em>HBCI4Java</em> selbstständig entweder über den Callback-Mechanismus vom Anwender oder durch
 * spezielle HBCI-Dialoge von der Bank.
 * </p>
 * <p>
 * Aus diesem Grund kann dieses Tool sowohl zum Testen eines schon existierenden Passports wie auch zum Einrichten eines neuen Passports verwendet werden, ohne
 * etwas am Programmcode zu ändern. In der gleichen Weise wie dieses Tool kann auch eine komplexere Anwendung <em>HBCI4Java</em> benutzen, in diesem Tool fehlt
 * nur das Hinzufügen von Geschäftsvorfällen zum HBCI-Dialog.
 * </p>
 * <p>
 * Beim Start des Programmes mit <code>java&nbsp;org.kapott.hbci.tools.InitAndTest</code> werden verschiedene Parameter über die Standardeingabe abgefragt.
 * Konkret handelt es sich dabei um alle HBCI-Parameter, die für die Initialisierung eines Passports und dessen Verwendung benötigt werden. Die HBCI-Parameter
 * werden mit den eingegeben Werten initialisiert. Anschließend wird ein Passport-Objekt erzeugt und ein "leerer" HBCI-Dialog (d.h. einer, der keine
 * Geschäftsvorfälle enthält) ausgeführt.
 * </p>
 * <p>
 * Bei diesem Vorgang sorgt <em>HBCI4Java</em> selbst dafür, dass alle relevanten Daten vorhanden sind bzw. initialisiert diese entspechend. Läuft das Programm
 * fehlerfrei durch, so ist sichergestellt, dass das benutzte Passport (=Sicherheitsmedium) korrekt initialisiert und funktionsbereit ist.
 * </p>
 */
public final class InitAndTest {
    ...
    public static void main(String[] args) throws IOException {
        try {
            ...
            /*
             * HBCIExecStatus ret=hbciHandle.execute(); System.out.println("ExecStatus"); System.out.println(ret.toString());
             * System.out.println("ExecStatusEnd"); System.out.println("ExecStatusError"); System.out.println(ret.getErrorString());
    ...
    private static void readPassportParams() throws IOException {
        ...
        if (type.equals("Anonymous")) {
            ...
            readParam("client.passport.DDV.libname.ddv", "/home/kleiner/projects/hbci2/chipcard/lib/libhbci4java-card-linux.so",
                            "the name of the library needed to use the CTAPI interface of your chipcard terminal");
            readParam("client.passport.DDV.libname.ctapi", "/usr/lib/libctapi-cyberjack.so",
            ...
            readParam("client.passport.DDV.softpin", "0",
            ...
            readParam("client.passport.PinTan.certfile",
                            null,
            ...
            readParam("client.passport.PinTan.proxyuser", null,
                            "when you need proxy authentication enter the username here; leave empty to be queried on demand");
            readParam("client.passport.PinTan.proxypass", null,
    ...
    private static void readHBCIVersion() throws IOException {
        ...
        readParam("client.passport.hbciversion.default", ((pversion.length() != 0) ? pversion : "210"),
                        "the hbci-version to be used; may be '201', '210', '220', '300' or 'plus'");
    }
