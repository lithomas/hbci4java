...
/**
 * <p>
 * Rewriter-Modul für falsche Informationen über TAN-Verfahren. Einige Banken mit HBCI+ (HBCI-PIN/TAN) -Unterstützung stellen fälschlicherweise in die BPD die
 * Information ein, dass das Sicherheitsverfahren "TAN" unterstützt wird. "TAN" ist aber kein gültiger Code für Sicherheitsmechanismen, so dass dieses
 * Rewriter-Modul die "TAN"-Information aus den BPD entfernt.
 * </p>
 * <p>
 * Ist dieses Modul aktiv, so muss auch das Modul "<code>Olly</code>" aktiv sein, weil einige hier vorgenommene Änderungen wiederum fehlerhafte Nachrichten
 * erzeugen, die aber durch "<code>Olly</code>" wieder korrigiert werden.
 * </p>
 */
public class RSecTypeTAN extends Rewrite {
    ...
    public String incomingClearText(String st, MsgGen gen) {
        ...
        MSG msg = MSGFactory.getInstance().createMSG(myMsgName, st, st.length(), gen, MSG.CHECK_SEQ, MSG.DONT_CHECK_VALIDS);
        ...
        for (int i = 0;; i++) {
            ...
            if (method.equals("TAN")) { // "TAN" ist ungültiger Bezeichner
                ...
                /*
                 * wenn mehr als eine SecMethod im Segment stand, dann braucht nur das eine der multiplen DEGs entfernt werden. Wenn aber nur die eine
                 * fehlerhafte Info enthalten war, dann muss das gesamte Segment entfernt werden, weil ein SecMethods-Segment ohne tatsächliche Daten über
                 * unterstützte SecMethods ungültig ist.
                 */
                ...
                if (number > 1) { // nur das eine fehlerhafte TAN:1 löschen
                    ...
                    /*
                     * der Fehler, der hier gemacht wird (nachfolgende Segment- Sequenznummern sind falsch), wird durch ein nachgeschaltetes Olly-Modul
                     * korrigiert
                     */
                }
