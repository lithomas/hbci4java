...
/**
 * Rewriter-Modul für Kontoauszüge der 1822direkt-Bank (und vielleicht andere). Die Kontoauszüge können nicht geparst werden, weil das verwendete SWIFT-Format
 * von dem in der HBCI-Spezifikation vorgeschriebenen Format abweicht. Dieses Modul korrigiert die Fehler in den Kontoauszugsdaten, so dass Kontoauszüge mit
...
public final class RKUmsDelimiters extends Rewrite {
    private String rewriteKUms(String st) {
        ...
        if (!temp.substring(temp.length() - 3, temp.length()).equals("\r\n-")) {
            ...
            if (posi >= 0) {
                if (posi != temp.length() - 1) {
                    ...
                    HBCIUtils.log("absolutely no ending sequence found - " + "maybe statement of account splitted at wrong position?", HBCIUtils.LOG_WARN);
                }
            } else {
                HBCIUtils.log("statement of account seems to be empty", HBCIUtils.LOG_WARN);
            }
        }
        ...
        if (!temp.toString().equals(st)) {
            ...
            HBCIUtils.log("wrongCRLF:" + wrongCRLF + " wrongDelimiterChars:" + wrongDelimiter + " wrongEnd:" + wrongEndSequence + " missingMinusBetweenCRLFs:"
                            + missingMinusBetweenCRLFs + " missingCRLFMinus:" + missingCRLFMinus, HBCIUtils.LOG_WARN);
        }
    ...
    public MSG incomingData(MSG msg, MsgGen gen) {
        ...
        for (Enumeration i = data.propertyNames(); i.hasMoreElements();) {
            ...
            if (key.startsWith(header) && key.indexOf("KUms") != -1 && key.endsWith(".booked")) {
                ...
                st = rewriteKUms(st);
                msg.propagateValue(msg.getName() + "." + key, "B" + st, SyntaxElement.DONT_TRY_TO_CREATE, SyntaxElement.ALLOW_OVERWRITE);
            }
        }
