...
/**
 * <p>
 * Repräsentation eines HBCI-Statuscodes. Objekte dieser Klasse stellen einen einzigen HBCI-Returncode dar, welcher aus einer Antwortnachricht von der Bank
 * extrahiert wurde.
 * </p>
 * <p>
 * Zu den hier bereitgestellten Informationen zählen neben den eigentlichen Status-Daten (Status-Code, Textmeldung) auch eine numerische Darstellung
 * <em>des</em> Teiles der ursprünglich gesendeten Nachricht, auf den sich diese Statusmeldung bezieht. Sofern das möglich ist, wird diese numerische
 * Darstellung zusätzlich in den Lowlevel-Namen des betreffenden Nachrichtenteils umgewandelt, so dass für den Anwender eine bessere Lokalisierung des Problems
 * möglich ist.
 * </p>
 */
public final class HBCIRetVal implements Serializable {
    ...
    public String code;
    /**
     * Segmentnummer in der gesendeten Nachricht, auf das sich dieser Rückgabewert bezieht. Falls es sich um einen globalen Rückgabewert handelt (d.h. einen,
     * der sich auf die komplette Nachricht bezieht), so ist dieser Wert <code>null</code>
     */
    public String segref;
    /**
     * Nummer des Datenelementes oder der Datenelementgruppe, auf das sich dieser Rückgabewert bezieht. Diese Information ist nicht in jedem Fall vorhanden
     * (z.B. wenn es sich um einen globalen Fehlercode handelt oder wenn sich der Rückgabewert auf ein komplettes Segment bezieht). In einem solchen Fall ist
     * dieser Wert <code>null</code>.
     */
    public String deref;
    ...
    /**
     * Lowlevel-Name des Nachrichtenelementes, auf das sich dieser Rückgabewert bezieht. Dieser Name kann nicht in jedem Fall bestimmt werden, der Wert dieses
     * Feldes kann dann auch <code>null</code> sein.
     */
     ...
     * 
     * @return einen String, der alle Informationen dieses Objektes kurz zusammenfasst.
     */
