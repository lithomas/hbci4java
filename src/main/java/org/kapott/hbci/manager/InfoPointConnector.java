...
public class InfoPointConnector {
    ...
    private void sendDataToServer(HBCIPassportInternal passport, String data) {
        ...
        HBCIUtilsInternal.getCallback().callback(passport, HBCICallback.NEED_INFOPOINT_ACK, HBCIUtilsInternal.getLocMsg("CALLB_INFOPOINT_ACK"),
                        HBCICallback.TYPE_BOOLEAN, retData);

        if (retData.length() == 0) {
            HBCIUtils.log("sending data about successfully received BPD to InfoPoint server", HBCIUtils.LOG_INFO);

            HBCIUtilsInternal.getCallback().status(passport, HBCICallback.STATUS_SEND_INFOPOINT_DATA, data);
    ...
    public void sendPublicKeys(HBCIPassportInternal passport, Properties msgData) {
        /*
         * TODO: activate this later Properties passportData=preparePassportData(passport); String xmlData=prepareXMLDocument("keysReceived", passportData,
         * msgData); sendDataToServer(passport,xmlData);
         */
    }
}
