...
/**
 * Uebernimmt das Umwandeln eines Flicker-Codes in die blinkende Darstellung. Da wir hier in HBCI4Java ja keinen GUI-spezifischen Code (Swing, SWT, etc.) haben,
 * uebernimmt die Klasse lediglich das Erzeugen der schwarzen und weissen Balken sowie das Timing. Sie ruft dann im Wiedergabe-Takt die Funktion paint() auf,
 * die ueberschrieben werden muss, um dort dann das eigentliche Zeichnen der 5 Balken durchzufuehren. Die paint-Funktion wird so ca. 10-20 mal pro Sekunde
 * aufgerufen, sollte die Ausgabe auf dem Bildschirm daher flott machen ;)
 */
public class FlickerRenderer {
     ...
     * 
     * @param code
     *            der zu rendernde Flicker-Code. Er wird von HBCI4Java ueber den Callback NEED_PT_TA uebergeben.
     * 
     *            Etwa so:
     * 
     *            case HBCICallback.NEED_PT_TAN: String flicker = retData.toString(); if (flicker != null && flicker.length() > 0) { MyFlickerRenderer = new
     *            FlickerRenderer(flicker) { // hier paint() ueberschreiben }; }
     */
     ...
     * 
     * @param hz
     *            die zu verwendende Taktfrequenz. Es werden nur Werte zwischen {@link FlickerRenderer#FREQUENCY_MIN} und {@link FlickerRenderer#FREQUENCY_MAX}
     *            akzeptiert.
     */
    ...
    /**
     * Startet das Rendering des Flicker-Codes. Die Funktion startet einen neuen Thread, kehrt also sofort zurueck.
     * 
     * Da diese Funktion einen neuen Thread startet und somit sofort zurueckkehrt, kann es ggf. noetig sein, eine Warteschleife zu implementieren. Hierzu kann
     * einfach die Funktion "waitFor()" aufgerufen werden. Sie pausiert solange, bis "stop()" augerufen wurde.
     * 
     * Beispiel:
     * 
     * FlickerRenderer renderer = new FlickerRenderer(meinCode) { public void paint(boolean b1,boolean b2,boolean b3,boolean b4,boolean b5) { // hier eigenen
     * Code zum Rendern einbauen }
     * 
    ...
    /**
     * Wird immer dann aufgerufen, wenn die 5 Balken der Flicker-Grafik aktualisiert werden sollen. Die 5 Boolean-Parameter legen die aktuell anzuzeigende
     * Codierung fest.
     * 
     ...
     * 
     * @param b1
     *            Balken 1. true=weiss, false=schwarz
     * @param b2
     *            Balken 2. true=weiss, false=schwarz
     * @param b3
     *            Balken 3. true=weiss, false=schwarz
     * @param b4
     *            Balken 4. true=weiss, false=schwarz
     * @param b5
    ...
    /**
     * Wird immer dann aufgerufen, nachdem der Flicker-Code einmal komplett an den TAN-Generator uebertragen wurde. Die Funktion wird zum ersten Mal NACH der
     * ersten Uebertragung aufgerufen und anschliessend nach jeder weiteren.
     * 
     * Die Default-Implementierung ist leer.
     * 
     * Fuer gewoehnlich wird die Funktion zur Darstellung nicht benoetigt. Sie kann aber zu Debugging-Zwecken verwendet werden oder zum automatischen Abbruch
     * nach einer definierten Anzahl von Uebertragungen. Sie muss also nicht ueberschrieben werden.
     * 
     * Die Funktion wird direkt im Flicker-Thread aufgerufen. Sie sollte daher auf keinen Fall irgendwas aufwaendiges machen, da das zum Ausbremsen der
     * Flicker-Uebertragung fuehren wuerde.
     * 
     * @param iterations
    ...
    /**
     * Kann verwendet werden, um den Aufrufer-Thread solange zu pausieren, bis "stop()" aufgerufen wurde. Damit kann warten, bis die Uebertragung abgeschlossen
     * ist.
     */
...
/**********************************************************************
 * $Log: FlickerRenderer.java,v $ Revision 1.6 2011/06/07 13:55:08 willuhn
 * 
