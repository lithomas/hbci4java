...
public final class HBCIUser implements IHandlerData {
    ...
    private void doDialogEnd(String dialogid, String msgnum, boolean signIt, boolean cryptIt, boolean needCrypt) {
        ...
        HBCIMsgStatus status = kernel.rawDoIt(!isAnon && signIt, !isAnon && cryptIt, !isAnon && HBCIKernelImpl.NEED_SIG, !isAnon && needCrypt);
    ...
    private void sendAndActivateNewUserKeys(HBCIKey[] sigKey, HBCIKey[] encKey) {
        try {
            ...
            if (!passport.hasMySigKey()) {
                ...
                HBCIMsgStatus ret = kernel.rawDoIt(HBCIKernelImpl.SIGNIT, HBCIKernelImpl.CRYPTIT, HBCIKernelImpl.NEED_SIG, HBCIKernelImpl.DONT_NEED_CRYPT);
                ...
                updateUPD(result);
                passport.saveChanges();
                HBCIUtilsInternal.getCallback().status(passport, HBCICallback.STATUS_DIALOG_INIT_DONE,
                                new Object[] { ret, result.getProperty("MsgHead.dialogid") });
                ...
                result = ret.getData();
                HBCIUtilsInternal.getCallback().status(passport, HBCICallback.STATUS_SEND_KEYS_DONE, ret);
                doDialogEnd(result.getProperty("MsgHead.dialogid"), "3", HBCIKernelImpl.SIGNIT, HBCIKernelImpl.CRYPTIT, HBCIKernelImpl.NEED_CRYPT);
            }
        } catch (Exception e) {
    ...
    private void triggerNewKeysEvent() {
        ...
        HBCIUtilsInternal.getCallback().callback(passport, HBCICallback.HAVE_NEW_MY_KEYS, HBCIUtilsInternal.getLocMsg("CALLB_NEW_USER_KEYS"),
                        HBCICallback.TYPE_NONE, new StringBuffer());
    ...
    public void fetchSysId() {
        try {
            ...
            doDialogEnd(result.getProperty("MsgHead.dialogid"), "2", HBCIKernelImpl.SIGNIT, HBCIKernelImpl.CRYPTIT, HBCIKernelImpl.NEED_CRYPT);
        } catch (Exception e) {
    ...
    public void fetchSigId() {
        try {
            ...
            while (true) {
                ...
                ret = kernel.rawDoIt(passport.hasMySigKey(), HBCIKernelImpl.CRYPTIT, HBCIKernelImpl.NEED_SIG, passport.hasMyEncKey());
            ...
            doDialogEnd(result.getProperty("MsgHead.dialogid"), "2", passport.hasMySigKey(), HBCIKernelImpl.CRYPTIT, passport.hasMyEncKey());
        } catch (Exception e) {
    ...
    public void fetchUPD() {
        try {
            ...
            while (true) {
                ...
                kernel.rawSet("ProcPrep.prodName", HBCIUtils.getParam("client.product.name", "HBCI4Java"));
                kernel.rawSet("ProcPrep.prodVersion", HBCIUtils.getParam("client.product.version", "2.5"));
                ret = kernel.rawDoIt(!isAnon && HBCIKernelImpl.SIGNIT, !isAnon && HBCIKernelImpl.CRYPTIT, !isAnon && HBCIKernelImpl.NEED_SIG, !isAnon
    ...
    public void lockKeys() {
        ...
        try {
            ...
            while (true) {
                ...
                status = kernel.rawDoIt(HBCIKernelImpl.SIGNIT, HBCIKernelImpl.CRYPTIT, HBCIKernelImpl.NEED_SIG, HBCIKernelImpl.NEED_CRYPT);
            ...
            status = kernel.rawDoIt(HBCIKernelImpl.SIGNIT, HBCIKernelImpl.CRYPTIT, HBCIKernelImpl.NEED_SIG, HBCIKernelImpl.DONT_NEED_CRYPT);
            if (!status.isOK())
                throw new ProcessException(HBCIUtilsInternal.getLocMsg("EXCMSG_LOCKFAILED"), status);
