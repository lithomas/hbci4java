...
public final class HBCIDialog {
    ...
    private List<ArrayList<HBCIJobImpl>> msgs; /*
                                                * this array contains all messages to be sent (excluding dialogInit and dialogEnd); each element of the
                                                * arrayList is again an ArrayList, where each element is one task (GV) to be sent with this specific message
                                                */
    ...
    /**
     * @brief Processing the DialogInit stage and updating institute and user data from the server (mid-level API).
     * 
     *        This method processes the dialog initialization stage of an HBCIDialog. It creates a new rawMsg in the kernel and processes it. The return values
     *        will be passed to appropriate methods in the @c institute and @c user objects to update their internal state with the data received from the
     *        institute.
     */
        ...
        try {
            ...
            if (ret.isOK()) {
                ...
                for (int i = 0; true; i++) {
                    ...
                    HBCIUtilsInternal.getCallback().callback(mainPassport, HBCICallback.HAVE_INST_MSG, msg.toString(), HBCICallback.TYPE_NONE,
                                    new StringBuffer());
                }
            }
    ...
    private HBCIMsgStatus[] doJobs() {
        ...
        for (int j = 0; j < nof_messages; j++) {
            ...
            while (true) {
                ...
                try {
                    HBCIUtils.log("generating custom msg #" + (j + 1) + " (loop " + (loop + 1) + ")", HBCIUtils.LOG_DEBUG);
    ...
    /**
     * führt einen kompletten dialog mit allen zu diesem dialog gehoerenden nachrichten/tasks aus.
     * 
     * bricht diese methode mit einer exception ab, so muessen alle nachrichten bzw. tasks, die noch nicht ausgeführt wurden, von der aufrufenden methode neu
     * erzeugt werden
     */
        ...
        try {
            ...
            if ((maxGVA > 0 && gva_counter > maxGVA) || (maxGVSegJob > 0 && gv_counter > maxGVSegJob) || (maxGVSegTotal > 0 && total_counter > maxGVSegTotal)) {
                if (maxGVSegTotal > 0 && total_counter > maxGVSegTotal) {
                    HBCIUtils.log("have to generate new message because current type of passport only allows " + maxGVSegTotal + " GV segs per message",
                                    HBCIUtils.LOG_INFO);
                } else {
                    HBCIUtils.log("have to generate new message because of BPD restrictions for number of tasks per message; adding job to this new message",
                                    HBCIUtils.LOG_INFO);
                }
                newMsg();
                gv_counter = 1;
