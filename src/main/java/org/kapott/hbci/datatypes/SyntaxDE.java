...
public abstract class SyntaxDE {
     ...
     * 
     *        In this constructor the data element will be initialized with the given value @p x. During initializing process the size constraints for this
     *        element will be verified. If verification fails, an IllegalArgumentException will be thrown.
     * @param x
     *            The string representation of the init value.
     * @param minsize
     *            The minimum size (string length) of the init value.
     * @param maxsize
    ...
    protected final void setContent(String st, int minsize, int maxsize) {
        /*
         * die stringlaenge muss zwischen minsize und maxsize liegen, im falle 'maxsize=0' kann sie beliebig gross sein
         */
        int len = st.length();
        if (len < minsize || (maxsize != 0 && len > maxsize)) {
            String msg = HBCIUtilsInternal.getLocMsg("EXC_DT_INV_STRINGLEN",
                            new Object[] { st, Integer.toString(len), Integer.toString(minsize), Integer.toString(maxsize) });
     ...
     * @param zero
     *            Just a dummy argument to overload the toString method.
     ...
     * @return den aktuellen Inhalt des SyntaxDE in der intern gespeicherten Form (human-readable)
     */
    ...
    /**
     * @internal
     * @brief Returns the index, where the next value in the HBCI message @c res starts.
     * 
     *        This method is needed when parsing HBCI messages. It checks the first character of String @p res. If this character is one of the HBCI-specific
     *        data-element-delimiters, 1 will be returned, else 0 (i.e. the return value is the index into the String @p res where we can start fetching the
     *        next syntax token)
     * @param res
     *            A part of an HBCI message to be parsed.
     ...
     * @p res starting at position @p startidx for the next occurence of an HBCI-specific data-element-delimiter and returns its position.
     * @param res
     *            Part of an HBCI-message to be parsed.
     * @param startidx
     *            Index position where to start looking for the next delimiter.
     * @return The position of the next delimiter character in @p res. If there is no next delimiter character, the return value will be the next character
