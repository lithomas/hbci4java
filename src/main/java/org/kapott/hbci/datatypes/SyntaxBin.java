...
public class SyntaxBin extends SyntaxDE {
     ...
     * 
     *           This method transforms a given number into its byte-representation in big-endian-format.
     * @param x
     *            the String representation of the number
     * @return a String, where each "character" is one byte of the big-endian-byte-representation of the given number
     */
     ...
     * 
     *           Es wird der erzeugte String zurückgegeben. Dazu wird das erste Zeichen des uebergebenen Strings ausgewertet. Ist dieses "N", so wird der
     *           uebergebene String als Integer-Wert interpretiert und in seine binaere Byte-Darstellung konvertiert. Bei "B" als erstem Zeichen wird der String
     *           as-is uebernommen.
     * @param x
    ...
    private static String expand(String x) {
        ...
        case 'B':
            ...
            break;
        default:
            throw new InvalidArgumentException(HBCIUtilsInternal.getLocMsg("EXC_DTBIN_NO_VALID_FORMAT", Character.toString(format)));
        }
     ...
     * @param st
     *            the string representing the complete datatype BIN
     * @return the length of the header-field in this string (i.e. the number of bytes making the @len@ part of the string
     */
     ...
     * 
     *           this method takes a string representing the value of a BIN-datatype and extracts the real value (the data making the element) from it. for this
     *           to achieve it removes the header containing the length of the data
     * @param st
     *            the content of an HBCI-BIN-datatype-field
