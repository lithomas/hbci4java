...
/**
 * <p>
 * Diese Klasse stellt die Basisklasse für alle "echten" Passport-Implementationen dar. Hier werden bereits einige Methoden implementiert sowie einige
...
public abstract class AbstractHBCIPassport implements HBCIPassportInternal, Serializable {
    ...
    protected boolean askForMissingData(boolean needCountry, boolean needBLZ, boolean needHost, boolean needPort, boolean needFilter, boolean needUserId,
                    boolean needCustomerId) {
        boolean dataChanged = false;

        if (needCountry && (getCountry() == null || getCountry().length() == 0)) {
        ...
        if (needBLZ && (getBLZ() == null || getBLZ().length() == 0)) {
        ...
        if (needHost && (getHost() == null || getHost().length() == 0)) {
            StringBuffer sb;
        ...
        if (needPort && (getPort() == null || getPort().intValue() == 0)) {
        ...
        if (needFilter && (getFilterType() == null || getFilterType().length() == 0)) {
        ...
        if (needUserId && (getUserId() == null || getUserId().length() == 0)) {
            StringBuffer sb = new StringBuffer();
        ...
        if (needCustomerId && (getStoredCustomerId() == null || getStoredCustomerId().length() == 0)) {
        ...
        if (upd != null) {
            for (int i = 0;; i++) {
                ...
                if ((st = upd.getProperty(header + ".KLimit.limittype")) != null) {
                    ...
                    limit.value = new Value(upd.getProperty(header + ".KLimit.BTG.value"), upd.getProperty(header + ".KLimit.BTG.curr"));
    ...
    public final String[] getSuppLangs() {
        ...
        if (bpd != null) {
            ...
            while ((header = HBCIUtilsInternal.withCounter("BPA.SuppLangs.lang", i)) != null && (value = bpd.getProperty(header)) != null) {
                temp.add(value);
                i++;
            }
    ...
    public final String[] getSuppVersions() {
        ...
        if (bpd != null) {
            ...
            while ((header = HBCIUtilsInternal.withCounter("BPA.SuppVersions.version", i)) != null && (value = bpd.getProperty(header)) != null) {
                temp.add(value);
                i++;
            }
    ...
    public final String[][] getSuppSecMethods() {
        ...
        if (bpd != null) {
            ...
            while ((header = HBCIUtilsInternal.withCounter("SecMethod.SuppSecMethods", i)) != null && (method = bpd.getProperty(header + ".method")) != null) {

                String header2;
                String version;
                int j = 0;

                while ((header2 = HBCIUtilsInternal.withCounter(header + ".version", j)) != null && (version = bpd.getProperty(header2)) != null) {
    ...
    public final String[][] getSuppCompMethods() {
        ...
        if (bpd != null) {
            ...
            while ((header = HBCIUtilsInternal.withCounter("CompMethod.SuppCompMethods", i)) != null && (method = bpd.getProperty(header + ".func")) != null) {
    ...
    /**
     * <p>
     * Erzeugt eine Instanz eines HBCIPassports und gibt diese zurück. Der Typ der erzeugten Passport-Instanz wird durch den Parameter <code>name</code>
     ...
     * </p>
     * <p>
     * Der zusätzliche Parameter <code>init</code> gibt ein Objekt an, welches bereits während der Instanziierung des Passport-Objektes in dessen internen
     * <code>clientData</code>-Datenstrukturen gespeichert wird (siehe {@link org.kapott.hbci.passport.HBCIPassport#setClientData(String,Object)}). Auf dieses
     * Objekt kann dann mit {@link org.kapott.hbci.passport.HBCIPassport#getClientData(String) getClientData("init")} zugegriffen werden. Ist
     * <code>init==null</code>), wo wird <code>init=name</code> gesetzt.
     * </p>
     * <p>
     * Beim Erzeugen eines Passport-Objektes tritt i.d.R. der {@link org.kapott.hbci.callback.HBCICallback Callback} <code>NEED_PASSPHRASE</code> auf, um nach
     * dem Passwort für das Einlesen der Schlüsseldatei zu fragen. Von der Callback-Methode eventuell zusätzlich benötigte Daten zu diesem Passport konnten bis
     * zu dieser Stelle noch nicht via <code>setClientData(...)</code> gesetzt werden, weil das Passport-Objekt noch gar nicht existierte. Für diesen Zweck gibt
     * es das <code>init</code>-Objekt, welches bereits beim Erzeugen des Passport-Objektes (und <em>vor</em> dem Aufrufen eines Callbacks) zu den zusätzlichen
     * Passport-Daten hinzugefügt wird (mit der id "<code>init</code>").
     * </p>
     * <p>
     * Eine beispielhafte (wenn auch nicht sehr praxisnahe) Anwendung dieses Features wird im Quelltext des Tools
     * {@link org.kapott.hbci.tools.AnalyzeReportOfTransactions} gezeigt. Zumindest das Prinzip sollte damit jedoch klar werden.
     * </p>
     * 
     * @param name
     *            Typ der zu erzeugenden Passport-Instanz
     * @param init
     *            Objekt, welches schon während der Passport-Erzeugung via <code>setClientData("init",init)</code> zu den Passport-Daten hinzugefügt wird.
    ...
    /**
     * Erzeugt eine Instanz eines HBCI-Passports. Der Typ der erzeugten Passport-Instanz wird hierbei dem Wert des HBCI-Parameters
     * <code>client.passport.default</code> entnommen. Gültige Werte für diesen HBCI-Parameter sind die gleichen wie beim Aufruf der Methode
     * {@link #getInstance(String)}.
     * 
     * @param init
     *            (siehe {@link #getInstance(String,Object)})
    ...
    protected SecretKey calculatePassportKey(boolean forSaving) {
        try {
            ...
            HBCIUtilsInternal.getCallback().callback(this, forSaving ? HBCICallback.NEED_PASSPHRASE_SAVE : HBCICallback.NEED_PASSPHRASE_LOAD,
                            forSaving ? HBCIUtilsInternal.getLocMsg("CALLB_NEED_PASS_NEW") : HBCIUtilsInternal.getLocMsg("CALLB_NEED_PASS"),
                            HBCICallback.TYPE_SECRET, passphrase);
        ...
        for (Enumeration e = bpd.propertyNames(); e.hasMoreElements();) {
            ...
            if (key.startsWith("Params") && key.endsWith(".SegHead.code")) {
                ...
                if (version.length() != 0) {
                    ...
                    if (knownVersion == null || Integer.parseInt(version) > Integer.parseInt(knownVersion)) {
    ...
    public Properties getJobRestrictions(String specname) {
        ...
        return getJobRestrictions(specname.substring(0, versionPos + 1), specname.substring(versionPos + 1));
    }
    ...
    public Properties getJobRestrictions(String gvname, String version) {
        ...
        for (Enumeration e = bpd.propertyNames(); e.hasMoreElements();) {
            ...
            if (key.startsWith("Params") && key.indexOf("." + searchstring + ".Par") != -1) {
                int searchIdx = key.indexOf(searchstring);
                result.setProperty(key.substring(key.indexOf(".", searchIdx + searchstring.length() + 4) + 1), bpd.getProperty(key));
            }
        }
