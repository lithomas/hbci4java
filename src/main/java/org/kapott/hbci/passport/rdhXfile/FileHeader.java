...
public class FileHeader extends TLV {
    ...
    public FileHeader(TLV tlv) {
        ...
        this.nof_iterations = ((data[offset + 25] & 0xFFL) << 24) | ((data[offset + 24] & 0xFFL) << 16) | ((data[offset + 23] & 0xFFL) << 8)
                        | ((data[offset + 22] & 0xFFL) << 0);

        this.salt = new byte[20];
        System.arraycopy(data, offset + 2, this.salt, 0, 20);
        HBCIUtils.log("file is a RDH-" + this.profileversion + "-file in version " + this.version, HBCIUtils.LOG_DEBUG);
    }
