...
public class HBCIAccount extends TLV {
    ...
    public static class UserKeys extends TLV {
        ...
        public Key getPublicKey() {
            try {
                KeySpec spec = new RSAPublicKeySpec(new BigInteger(+1, modulus), new BigInteger(+1, exponent));
        ...
        public Key getPrivateKey() {
            try {
                RSAPrivateCrtKey2 k = new RSAPrivateCrtKey2(new BigInteger(+1, p), new BigInteger(+1, q), new BigInteger(+1, dP), new BigInteger(+1, dQ),
    ...
    private HBCIKey[] getUserKeys(String keytype) {
        ...
        for (Iterator<UserKeys> i = userkeys.iterator(); i.hasNext();) {
            ...
            if (key.getKeyType().equals(keytype)) {
                ...
                ret[0] = new HBCIKey(getCountry(), getBLZ(), getUserId(), Integer.toString(key.getKeyNum()), Integer.toString(key.getKeyVersion()),
                                key.getPublicKey());

                ret[1] = new HBCIKey(getCountry(), getBLZ(), getUserId(), Integer.toString(key.getKeyNum()), Integer.toString(key.getKeyVersion()),
                                key.getPrivateKey());
            }
        }
