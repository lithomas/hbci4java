...
/**
 * <p>
 * Passport-Implementation für anonyme Zugänge. Bei dieser Passport-Variante handelt es sich nicht um einen "echten" HBCI-Zugang. Statt dessen handelt es sich
 * hierbei um einen anonymen Zugang, wie er von einigen wenigen Banken angeboten wird. Bei einem anonymen Zugang werden die Nachrichten nicht kryptographisch
 * gesichert (also keine Signaturen und keine Verschlüsselung). Aus diesem Grund können mit diesem Zugang maximal die Geschäftsvorfälle abgewickelt werden, die
 * keine Signatur benötigten (z.B. Gastmeldung an Bank senden).
 * </p>
 * <p>
 * Beim Einrichten eines solchen Passport-Objektes werden also keine Nutzer- bzw. Kunden-Kennungen abgefragt (diese sind automatisch auf die Kennungen für den
 * anonymen Zugang eingestellt). Außerdem findet keine Synchronisierung der Schlüssel zwischen Bank und Kunde statt, da bei dieser Zugangsform keine Schlüssel
 * verwendet werden.
 * </p>
 * <p>
 * Eine HBCI-Anwendung kann ein Anonymous-Passport genauso verwenden wie ein "normales" Passport. Alle Abweichungen, die bei der Durchführung anonymer Dialoge
 * zu beachten sind, werden völlig transparent von <em>HBCI4Java</em> umgesetzt.
 * </p>
 * <p>
 * Gegenwärtig können mit Anonymous-Passports noch keine anonymen Geschäftsvorfälle ausgelöst werden. Diese Passport-Variante kann also nur für einen "leeren"
 * HBCI-Dialog verwendet werden, der aus (anonymer) Dialog-Initialisierung und (anonymem) Dialog-Ende besteht. Damit kann zumindest die Verfügbarkeit des
 * HBCI-Servers bzw. von anonymen Zugängen überprüft werden.
 * </p>
 */
public class HBCIPassportAnonymous extends AbstractHBCIPassport {
    ...
    protected final static byte[] CIPHER_SALT = { (byte) 0x26, (byte) 0x19, (byte) 0x38, (byte) 0xa7, (byte) 0x99, (byte) 0xbc, (byte) 0xf1, (byte) 0x55 };
    ...
    public byte[] hash(byte[] data) {
        /*
         * the function hash-before-sign has nothing to do here, so we simply return the original message
         */
        return data;
    }
