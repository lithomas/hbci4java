...
public abstract class AbstractRDHPassport extends AbstractHBCIPassport {
    ...
    public boolean isSupported() {
        ...
        if (getBPD() != null) {
            ...
            for (int i = 0; i < methods.length; i++) {
                ...
                if (method.equals("RDH") && (noPassportVersion || passportVersion.equals(version))) {
    ...
    public byte[] hash(byte[] data) {
        /*
         * data is the plaintext message to be signed. In most cases, we do NOT have to execute an explicit hashing here, because most signature algorithms
         * already INCLUDE their own hashing step. The only exception is RDH-10, where we have to sign the HASH value, not the HBCI message itself, so in this
         * case we have to execute one round of hashing here
         */
