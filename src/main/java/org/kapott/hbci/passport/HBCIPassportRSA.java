...
public class HBCIPassportRSA extends AbstractRDHPassport implements HBCIPassportChipcard {
    ...
    public String getProfileVersion() {
        ...
        if (result == null) {
            ...
            /*
             * TODO: do not use the hbci-version stored in the passport, but the hbci version of the current HBCIHandler associated with this passport. This
             * will be easy in HBCI4Java-3, but in HBCI4Java-2 this is an ugly problem - broken by design
             */
            if (getHBCIVersion().length() != 0 && !getHBCIVersion().startsWith("3")) { // TODO: support FinTS-4, too
                ...
                if (key != null) {
                    ...
                    if (key != null && (key.num.equals("1") || key.num.equals("2") || key.num.equals("10"))) {
                        ...
                        HBCIUtils.log("using RDH profile '" + result + "' taken from supported profiles (BPD)", HBCIUtils.LOG_DEBUG);
                    }
                }
            }
        } else {
    ...
    protected void ctReadKeyData() {
        ...
        if (keyData[0].getStatus() == 0x10 && keyData[0].getKeyType() == 0x53) {
            ...
            setInstSigKey(new HBCIKey(getCountry(), getBLZ(), getBankId(), Integer.toString(keyData[0].getKeyNum()), Integer.toString(keyData[0]
                            .getKeyVersion()), keyData[0].getPublicKey()));
        }

        if (keyData[1].getStatus() == 0x10 && keyData[1].getKeyType() == 0x56) {
            HBCIUtils.log("found valid instEncKey ", HBCIUtils.LOG_DEBUG);
            setInstEncKey(new HBCIKey(getCountry(), getBLZ(), getBankId(), Integer.toString(keyData[1].getKeyNum()), Integer.toString(keyData[1]
                            .getKeyVersion()), keyData[1].getPublicKey()));
        }

        if (keyData[2].getStatus() == 0x10 && keyData[2].getKeyType() == 0x53) {
            HBCIUtils.log("found valid myPublicSigKey ", HBCIUtils.LOG_DEBUG);
            setMyPublicSigKey(new HBCIKey(getCountry(), getBLZ(), getUserId(), Integer.toString(keyData[2].getKeyNum()), Integer.toString(keyData[2]
                            .getKeyVersion()), keyData[2].getPublicKey()));
        }

        if (keyData[3].getStatus() == 0x10 && keyData[3].getKeyType() == 0x56) {
            HBCIUtils.log("found valid myPublicEncKey ", HBCIUtils.LOG_DEBUG);
            setMyPublicEncKey(new HBCIKey(getCountry(), getBLZ(), getUserId(), Integer.toString(keyData[3].getKeyNum()), Integer.toString(keyData[3]
                            .getKeyVersion()), keyData[3].getPublicKey()));
        }
    }
