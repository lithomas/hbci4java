...
/**
 * Implementierung eines DDV-Passports, welcher intern die neue Chipkarten-API "javax.smartcardio" von Java 6 verwendet. Die Implementierung basiert auf dem
 * OCF-Code von HBCI4Java 2.5.8.
 */
public class HBCIPassportDDVPCSC extends HBCIPassportDDV {
    ...
    protected void ctReadKeyData() {
        ...
        this.setInstSigKey(new HBCIKey(getCountry(), getBLZ(), getUserId(), Integer.toString(keyData[0].num), Integer.toString(keyData[0].version), null));

        this.setInstEncKey(new HBCIKey(getCountry(), getBLZ(), getUserId(), Integer.toString(keyData[1].num), Integer.toString(keyData[1].version), null));
    }
