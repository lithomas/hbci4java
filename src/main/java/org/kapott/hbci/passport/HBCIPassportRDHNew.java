...
/**
 * <p>
 * Passport-Klasse für RDH-Zugänge mit Sicherheitsmedium "Datei". Bei dieser Variante werden sowohl die HBCI-Zugangsdaten wie auch die kryptografischen
 * Schlüssel für die Signierung/Verschlüsselung der HBCI-Nachrichten in einer Datei gespeichert. Der Dateiname kann dabei beliebig vorgegeben werden. Da diese
 * Datei vertrauliche Informationen enthält, wird der Inhalt verschlüsselt abgespeichert. Vor dem Erzeugen bzw. Einlesen wird via Callback-Mechanismus nach
 * einem Passwort gefragt, aus dem der Schlüssel zur Verschlüsselung/Entschlüsselung der Schlüsseldatei berechnet wird.
 * </p>
 * <p>
 * Wie auch bei {@link org.kapott.hbci.passport.HBCIPassportDDV} werden in der Schlüsseldatei zusätzliche Informationen gespeichert. Dazu gehören u.a. die BPD
 * und die UPD sowie die HBCI-Version, die zuletzt mit diesem Passport benutzt wurde. Im Gegensatz zu den "Hilfsdateien" bei DDV-Passports darf die
 * Schlüsseldatei bei RDH-Passports aber niemals manuell gelöscht werden, da dabei auch die kryptografischen Schlüssel des Kunden verlorengehen. Diese können
 * nicht wieder hergestellt werden, so dass in einem solchen Fall ein manuelles Zurücksetzes des HBCI-Zuganges bei der Bank erfolgen muss!
 * </p>
 * <p>
 * Die Schlüsseldateien, die <em>HBCI4Java</em> mit dieser Klasse erzeugt und verwaltet, sind <b>nicht kompatibel</b> zu den Schlüsseldateien anderer
 * HBCI-Software (z.B. VR-NetWorld o.ä.). Es ist also nicht möglich, durch Auswahl des Sicherheitsverfahrens "RDH" oder "RDHNew" und Angabe einer schon
 * existierenden Schlüsseldatei, die mit einer anderen HBCI-Software erstellt wurde, diese Schlüsseldatei unter <em>HBCI4Java</em> zu benutzen! Es ist jedoch im
 * Prinzip möglich, mit der "anderen" Software die Kundenschlüssel sperren zu lassen und anschließend mit <em>HBCI4Java</em> eine völlig neue Schlüsseldatei zu
 * erzeugen. Das hat aber zwei Nachteile: Zum einen muss nach dem Neuerzeugen der Schlüsseldatei auch ein neuer INI-Brief erzeugt und an die Bank gesandt
 * werden, um die neuen Schlüssel freischalten zu lassen. Außerdem lässt sich natürlich die <em>HBCI4Java</em>-Schlüsseldatei nicht mehr in der "anderen"
 * HBCI-Software benutzen. Ein Parallel-Betrieb verschiedener HBCI-Softwarelösungen, die alle auf dem RDH-Verfahren mit Sicherheitsmedium "Datei" (oder
 * Diskette) basieren, ist meines Wissens nicht möglich.
 * </p>
 * <p>
 * Ein weiterer Ausweg aus diesem Problem wäre, eine technische Beschreibung des Formates der Schlüsseldateien der "anderen" HBCI-Software zu besorgen und diese
 * dem <a href="mailto:hbci4java@kapott.org">Autor</a> zukommen zu lassen, damit eine Passport-Variante implementiert werden kann, die mit Schlüsseldateien
 * dieser "anderen" Software arbeiten kann.
 * </p>
 * 
        ...
        if (base != null) {
            ...
            for (int i = 0; i < len; i++) {
                ...
                if (n.getNodeType() == Node.ELEMENT_NODE) {
                    ret.setProperty(((Element) n).getAttribute("name"), ((Element) n).getAttribute("value"));
                }
            }
        }
            ...
            if (n.getNodeType() == Node.ELEMENT_NODE) {
                ...
                if (keynode.getAttribute("owner").equals(owner) && keynode.getAttribute("type").equals(type) && keynode.getAttribute("part").equals(part)) {
                    ...
                    if (part.equals("public")) {
                        RSAPublicKeySpec spec = new RSAPublicKeySpec(new BigInteger(getElementValue(keynode, "modulus")), new BigInteger(getElementValue(
                                        keynode, "exponent")));
                        ...
                        if (privexponent == null) {
                            ...
                            RSAPrivateCrtKeySpec spec = new RSAPrivateCrtKeySpec(new BigInteger(modulus), new BigInteger(pubexponent), new BigInteger(
                                            privexponent), new BigInteger(p), new BigInteger(q), new BigInteger(dP), new BigInteger(dQ), new BigInteger(qInv));
                            key = KeyFactory.getInstance("RSA").generatePrivate(spec);
                        }
                    }
        ...
        if (ret.length() == 0) {
            ...
            if (hasMySigKey()) {
                HBCIUtils.log("found user sig key in passport file, but no profile version, "
                                + "so I guess it is an old RDHnew file, which always stored RDH-1 keys", HBCIUtils.LOG_DEBUG);
                // es gibt Schlüssel, aber keine profilVersion, also haben wir
                // gerade ein altes file gelesen, in dem diese Info noch nicht
                // drinstand
                ret = "1";
            } else {
                HBCIUtils.log("no user keys found in passport - so we use the highest available profile", HBCIUtils.LOG_DEBUG);
                ...
                for (int i = 0; i < methods.length; i++) {
                    ...
                    if (method.equals("RDH") && (version == 1 || version == 2 || version == 10)) {
                ...
                HBCIUtils.log("using RDH profile " + ret + " taken from supported profiles (BPD)", HBCIUtils.LOG_DEBUG);
            }
        }
