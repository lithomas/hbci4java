...
public abstract class AbstractPinTanPassport extends AbstractHBCIPassport {
                        ...
                        if (subkey.startsWith("ParTAN2Step") && subkey.endsWith(".secfunc")) {
                            ...
                            if (prev != null) {
                                ...
                                if (prevVersion > segVersion) {
                                    HBCIUtils.log("found another twostepmech " + secfunc + " in segversion " + segVersion
                                                    + ", allready have one in segversion " + prevVersion + ", ignoring segversion " + segVersion,
                                                    HBCIUtils.LOG_DEBUG);
                                    continue;
                                }
                            }
    ...
    private void searchFor3920s(HBCIRetVal[] rets) {
        ...
        for (int i = 0; i < l; i++) {
            ...
            if (ret.code.equals("3920")) {
                ...
                HBCIUtils.log("autosecfunc: found 3920 in response - updated list of allowed twostepmechs with " + allowedTwostepMechanisms.size() + " entries",
                                HBCIUtils.LOG_DEBUG);
            }
        }
    }
    ...
    public boolean postInitResponseHook(HBCIMsgStatus msgStatus, boolean anonDialog) {
        ...
        if (!anonDialog) {
            ...
            if (!oldTANMethod.equals(updatedTANMethod)) {
                ...
                restart_needed = true;
                HBCIUtils.log("autosecfunc: after this dialog-init we had to change selected pintan method from " + oldTANMethod + " to " + updatedTANMethod
                                + ", so a restart of this dialog is needed", HBCIUtils.LOG_INFO);
            }
        }
    ...
    private boolean isOneStepAllowed() {
        ...
        if (bpd != null) {
            for (Enumeration e = bpd.propertyNames(); e.hasMoreElements();) {
                ...
                if (key.startsWith("Params")) {
                    String subkey = key.substring(key.indexOf('.') + 1);
                    if (subkey.startsWith("TAN2StepPar") && subkey.endsWith(".can1step")) {
    ...
    /**
     * Kann vor <code>new HBCIHandler()</code> aufgerufen werden, um zu erzwingen, dass die Liste der unterstützten PIN/TAN-Sicherheitsverfahren neu vom Server
     * abgeholt wird und evtl. neu vom Nutzer abgefragt wird.
     */
    ...
    public String getCurrentTANMethod(boolean recheckSupportedSecMechs) {
        ...
        if (currentTANMethod == null || recheckSupportedSecMechs) {
            ...
            if (options.size() == 1) {
                ...
                HBCIUtils.log("autosecfunc: there is only one pintan method (" + autoSelection + ") supported - choosing this automatically",
                                HBCIUtils.LOG_DEBUG);
                if (currentTANMethod != null && !autoSelection.equals(currentTANMethod)) {
                    HBCIUtils.log("autosecfunc: currently selected method (" + currentTANMethod + ") differs from auto-selected method (" + autoSelection + ")",
                                    HBCIUtils.LOG_DEBUG);
                }
                ...
                if (currentTANMethod != null) {
                    ...
                    if (!ok) {
                        HBCIUtils.log("autosecfunc: currently selected pintan method (" + currentTANMethod
                                        + ") not in list of supported methods - resetting current selection", HBCIUtils.LOG_DEBUG);
                ...
                if (currentTANMethod == null || this.currentTANMethodWasAutoSelected) {
                    ...
                    if (allowedTwostepMechanisms.size() == 0 && getPersistentData("_authed_dialog_executed") == null) {
                        ...
                        String autoSelection = (options.get(0))[0];
                        HBCIUtils.log("autosecfunc: there are "
                                        + options.size()
                                        + " pintan methods supported, but we don't know which of them are allowed for the current user, so we automatically choose "
                                        + autoSelection, HBCIUtils.LOG_DEBUG);
                        setCurrentTANMethod(autoSelection);
                        ...
                        // callback erzeugen
                        HBCIUtilsInternal.getCallback().callback(this, HBCICallback.NEED_PT_SECMECH, "*** Select a pintan method from the list",
                                        HBCICallback.TYPE_TEXT, retData);
                ...
                // wir automatisch 999
                HBCIUtils.log("autosecfunc: absolutely no information about allowed pintan methods available - automatically falling back to 999",
                                HBCIUtils.LOG_DEBUG);
    ...
    public String getPinTanInfo(String code) {
        ...
        if (bpd != null) {
            ...
            for (Enumeration e = bpd.propertyNames(); e.hasMoreElements();) {
                ...
                if (key.startsWith("Params") && key.substring(key.indexOf(".") + 1).startsWith("PinTanPar") && key.indexOf(".ParPinTan.PinTanGV") != -1
                ...
                } else if (key.startsWith("Params") && key.endsWith(".SegHead.code")) {
    ...
    private String getOrderHashMode() {
        ...
        if (bpd != null) {
            for (Enumeration e = bpd.propertyNames(); e.hasMoreElements();) {
                ...
                if (key.startsWith("Params")) {
                    String subkey = key.substring(key.indexOf('.') + 1);
                    if (subkey.startsWith("TAN2StepPar") && subkey.endsWith(".orderhashmode")) {
                        ret = bpd.getProperty(key);
                        break;
                    }
                }
    ...
    private void applyTanMedia(GVTAN2Step hktan) {
        ...
        if (hktan_version >= 3) {
            ...
            if (needed.equals("2")) {
                ...
                HBCIUtilsInternal.getCallback().callback(this, HBCICallback.NEED_PT_TANMEDIA, "*** Enter the name of your TAN media", HBCICallback.TYPE_TEXT,
                                retData);
