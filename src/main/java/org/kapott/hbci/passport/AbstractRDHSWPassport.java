...
public abstract class AbstractRDHSWPassport extends AbstractRDHPassport {
    ...
    public byte[] sign(byte[] data) {
        /*
         * data is the result from the hash() method. In most cases, this is simply the hbci message to be signed, because the signature algorithms used here
         * (iso-9796-1, iso-9796-2, pkcs#1-pss) INCLUDE the hash-step, so it must not be done manually before. the only exception for this is is RDH-10 where an
         * extra round of hashing must be done before using PKCS#1-PSS
         */
