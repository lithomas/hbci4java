...
/**
 * <p>
 * Passport-Klasse für die Verwendung von RDH-2- und RDH-10-Schlüsseldateien mit <em>HBCI4Java</em>. RDH-2/10-Schlüsseldateien sind Schlüsseldateien für
 * RDH-Zugänge, die von anderer HBCI-Software erzeugt und verwendet werden (z.B. von <em>VR-NetWorld</em>). Soll eine solche Schlüsseldatei sowohl mit der
 * anderen Software als auch mit <em>HBCI4Java</em> verwendet werden, so kann das mit dieser Passport-Variante geschehen.
 * </p>
 */
public class HBCIPassportRDHXFile extends AbstractRDHSWFileBasedPassport {
        ...
        if (init) {
            ...
            try {
                ...
                if (hbciAccounts.length > 1) {
                    ...
                    HBCIUtilsInternal.getCallback().callback(this, HBCICallback.NEED_SIZENTRY_SELECT, "*** select one of the following entries",
                                    HBCICallback.TYPE_TEXT, possibilities);
    ...
    public void saveChanges() {
        try {
            if (this.passphrase == null) {
                StringBuffer retData = new StringBuffer();
                HBCIUtilsInternal.getCallback().callback(this, HBCICallback.NEED_PASSPHRASE_SAVE, HBCIUtilsInternal.getLocMsg("CALLB_NEED_PASS"),
                                HBCICallback.TYPE_SECRET, retData);
                // TODO: passwort-bedingungen nach spez. prüfen
                LogFilter.getInstance().addSecretData(retData.toString(), "X", LogFilter.FILTER_SECRETS);
                setPassphrase(retData.toString().getBytes());
