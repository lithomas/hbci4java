 ...
 * </p>
 * <p>
 * Bei diesem Verfahren werden die Nachrichten auf HBCI-Ebene nicht mit kryptografischen Verfahren signiert oder verschlüsselt. Als "Signatur" werden statt
 * dessen TANs zusammen mit einer PIN verwendet. Die PIN wird dabei in <em>jeder</em> HBCI-Nachricht als Teil der "Signatur" eingefügt, doch nicht alle
 * Nachrichten benötigen eine TAN. Eine TAN wird nur bei der Übermittlung bestimmter Geschäftsvorfälle benötigt. Welche GV das konkret sind, ermittelt
 * <em>HBCI4Java</em> automatisch aus den BPD. Für jeden GV, der eine TAN benötigt, wird diese via Callback abgefragt und in die Nachricht eingefügt.
 * </p>
 * <p>
 * Die Verschlüsselung der Nachrichten bei der Übertragung erfolgt auf einer höheren Transportschicht. Die Nachrichten werden nämlich nicht direkt via TCP/IP
 * übertragen, sondern in das HTTP-Protokoll eingebettet. Die Verschlüsselung der übertragenen Daten erfolgt dabei auf HTTP-Ebene (via SSL = HTTPS).
 * </p>
 * <p>
 * Wie auch bei {@link org.kapott.hbci.passport.HBCIPassportRDH} wird eine "Schlüsseldatei" verwendet. In dieser werden allerdings keine kryptografischen
 * Schlüssel abgelegt, sondern lediglich die Zugangsdaten für den HBCI-Server (Hostadresse, Nutzerkennung, usw.) sowie einige zusätzliche Daten (BPD, UPD,
 * zuletzt benutzte HBCI-Version). Diese Datei wird vor dem Abspeichern verschlüsselt. Vor dem Erzeugen bzw. erstmaligen Einlesen wird via Callback nach einem
 * Passwort gefragt, aus welchem der Schlüssel für die Verschlüsselung der Datei berechnet wird
 * </p>
 */
public class HBCIPassportPinTan extends AbstractPinTanPassport {
    ...
    public byte[] hash(byte[] data) {
        /*
         * there is no hashing before signing, so we return the original message, which will later be "signed" by sign()
         */
        return data;
    }
    ...
    public byte[] sign(byte[] data) {
        try {
            ...
            if (getPIN() == null) {
                ...
                HBCIUtilsInternal.getCallback().callback(this, HBCICallback.NEED_PT_PIN, HBCIUtilsInternal.getLocMsg("CALLB_NEED_PTPIN"),
            ...
            if (pintanMethod.equals(Sig.SECFUNC_SIG_PT_1STEP)) {
                ...
                while (tok.hasMoreTokens()) {
                    ...
                    if (info.equals("J")) {
                        ...
                        if (tan.length() == 0) {
                            ...
                            StringBuffer s = new StringBuffer();
                            HBCIUtilsInternal.getCallback().callback(this, HBCICallback.NEED_PT_TAN, HBCIUtilsInternal.getLocMsg("CALLB_NEED_PTTAN"),
                ...
                if (challenge == null) {
                    ...
                    HBCIUtilsInternal.getCallback().callback(this, HBCICallback.NEED_PT_TAN,
    ...
    /**
     * Versucht, aus Challenge und Challenge HHDuc den Flicker-Code zu extrahieren und ihn in einen flickerfaehigen Code umzuwandeln. Nur wenn tatsaechlich ein
     * gueltiger Code enthalten ist, der als HHDuc-Code geparst und in einen Flicker-Code umgewandelt werden konnte, liefert die Funktion den Code. Sonst immer
     * NULL.
     * 
     * @param challenge
     *            der Challenge-Text. Das DE "Challenge HHDuc" gibt es erst seit HITAN4. Einige Banken haben aber schon vorher optisches chipTAN gemacht. Die
     *            haben das HHDuc dann direkt im Freitext des Challenge mitgeschickt (mit String-Tokens zum Extrahieren markiert). Die werden vom
