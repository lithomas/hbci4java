...
public final class Sig {
    ...
    private void fillSigHead(SEG sighead) {
        ...
        sighead.propagateValue(sigheadName + ".secfunc", u_secfunc, SyntaxElement.DONT_TRY_TO_CREATE, SyntaxElement.DONT_ALLOW_OVERWRITE);
        sighead.propagateValue(sigheadName + ".seccheckref", seccheckref, SyntaxElement.DONT_TRY_TO_CREATE, SyntaxElement.DONT_ALLOW_OVERWRITE);
        /*
         * TODO: enable this later (when other range types are supported) sighead.propagateValue(sigheadName+".range",range,false);
         */
        sighead.propagateValue(sigheadName + ".role", u_role, SyntaxElement.DONT_TRY_TO_CREATE, SyntaxElement.DONT_ALLOW_OVERWRITE);
        sighead.propagateValue(sigheadName + ".SecIdnDetails.func", (msg.getName().endsWith("Res") ? "2" : "1"), SyntaxElement.DONT_TRY_TO_CREATE,
                        SyntaxElement.DONT_ALLOW_OVERWRITE);
        if (u_cid.length() != 0) {
            // DDV
            sighead.propagateValue(sigheadName + ".SecIdnDetails.cid", "B" + u_cid, SyntaxElement.DONT_TRY_TO_CREATE, SyntaxElement.DONT_ALLOW_OVERWRITE);
        } else {
            // RDH und PinTan
            sighead.propagateValue(sigheadName + ".SecIdnDetails.sysid", u_sysid, SyntaxElement.DONT_TRY_TO_CREATE, SyntaxElement.DONT_ALLOW_OVERWRITE);
        }
        sighead.propagateValue(sigheadName + ".SecTimestamp.date", HBCIUtils.date2StringISO(d), SyntaxElement.DONT_TRY_TO_CREATE,
                        SyntaxElement.DONT_ALLOW_OVERWRITE);
        sighead.propagateValue(sigheadName + ".SecTimestamp.time", HBCIUtils.time2StringISO(d), SyntaxElement.DONT_TRY_TO_CREATE,
                        SyntaxElement.DONT_ALLOW_OVERWRITE);

        sighead.propagateValue(sigheadName + ".secref", u_sigid, SyntaxElement.DONT_TRY_TO_CREATE, SyntaxElement.DONT_ALLOW_OVERWRITE);

        sighead.propagateValue(sigheadName + ".HashAlg.alg", u_hashalg, SyntaxElement.DONT_TRY_TO_CREATE, SyntaxElement.DONT_ALLOW_OVERWRITE);
        sighead.propagateValue(sigheadName + ".SigAlg.alg", u_sigalg, SyntaxElement.DONT_TRY_TO_CREATE, SyntaxElement.DONT_ALLOW_OVERWRITE);
        sighead.propagateValue(sigheadName + ".SigAlg.mode", u_sigmode, SyntaxElement.DONT_TRY_TO_CREATE, SyntaxElement.DONT_ALLOW_OVERWRITE);

        sighead.propagateValue(sigheadName + ".KeyName.KIK.country", u_keycountry, SyntaxElement.DONT_TRY_TO_CREATE, SyntaxElement.DONT_ALLOW_OVERWRITE);
        sighead.propagateValue(sigheadName + ".KeyName.KIK.blz", u_keyblz, SyntaxElement.DONT_TRY_TO_CREATE, SyntaxElement.DONT_ALLOW_OVERWRITE);
        sighead.propagateValue(sigheadName + ".KeyName.userid", u_keyuserid, SyntaxElement.DONT_TRY_TO_CREATE, SyntaxElement.DONT_ALLOW_OVERWRITE);
        sighead.propagateValue(sigheadName + ".KeyName.keynum", u_keynum, SyntaxElement.DONT_TRY_TO_CREATE, SyntaxElement.DONT_ALLOW_OVERWRITE);
        sighead.propagateValue(sigheadName + ".KeyName.keyversion", u_keyversion, SyntaxElement.DONT_TRY_TO_CREATE, SyntaxElement.DONT_ALLOW_OVERWRITE);

        sighead.propagateValue(sigheadName + ".SecProfile.method", passports.getMainPassport().getProfileMethod(), SyntaxElement.DONT_TRY_TO_CREATE,
                        SyntaxElement.DONT_ALLOW_OVERWRITE);
        sighead.propagateValue(sigheadName + ".SecProfile.version", passports.getMainPassport().getProfileVersion(), SyntaxElement.DONT_TRY_TO_CREATE,
                        SyntaxElement.DONT_ALLOW_OVERWRITE);
    }
    ...
    private void fillSigTail(SEG sighead, SEG sigtail) {
        ...
        sigtail.propagateValue(sigtailName + ".seccheckref", sighead.getValueOfDE(sighead.getPath() + ".seccheckref"), SyntaxElement.DONT_TRY_TO_CREATE,
                        SyntaxElement.DONT_ALLOW_OVERWRITE);
    }

    /*
     * daten zusammensammeln, die signiert werden müssen; idx gibt dabei an, die wievielte signatur erzeugt werden soll - wird benötigt, um festzustellen,
            ...
            if (dontsignAttr.length() == 0) {
                try {
                    ...
                    for (int idx = 0; idx < numOfPassports; idx++) {
                        ...
                        /*
                         * first calculate hash-result, then sign the hashresult. In most cases, the hash() step will be executed by the signature algorithm, so
                         * the hash() call returns the message as-is. Currently the only exception is PKCS#1-10, where an extra round of hashing must be
                         * executed before applying the signature process
                         */
                        ...
                        if (passport.needUserSig()) {
                            ...
                            if (pos != -1) {
                                ...
                                msg.propagateValue(sigtail.getPath() + ".UserSig.pin", pin, SyntaxElement.DONT_TRY_TO_CREATE,
                                                SyntaxElement.DONT_ALLOW_OVERWRITE);

                                if (pos < pintan.length() - 1) {
                                    String tan = pintan.substring(pos + 1);
                                    msg.propagateValue(sigtail.getPath() + ".UserSig.tan", tan, SyntaxElement.DONT_TRY_TO_CREATE,
                                                    SyntaxElement.DONT_ALLOW_OVERWRITE);
                                }
                            }
                        } else { // normale signatur
                            msg.propagateValue(sigtail.getPath() + ".sig", "B" + new String(signature, Comm.ENCODING), SyntaxElement.DONT_TRY_TO_CREATE,
                                            SyntaxElement.DONT_ALLOW_OVERWRITE);
                        }
                ...
                HBCIUtils.log("did not sign - message does not want to be signed", HBCIUtils.LOG_DEBUG);

            ret = true;
        } else
            HBCIUtils.log("can not sign - no signature key available", HBCIUtils.LOG_WARN);
    ...
    private void readSigHead() {
        ...
        /*
         * if (u_secfunc.equals("2")) { // DDV u_cid=msg.getValueOfDE(sigheadName+".SecIdnDetails.cid"); if (!u_cid.equals(mainPassport.getCID())) { String
         * errmsg=HBCIUtilsInternal.getLocMsg("EXCMSG_CRYPTCIDFAIL"); if (!HBCIUtilsInternal.ignoreError(null,"client.errors.ignoreSignErrors",errmsg)) throw
         * new HBCI_Exception(errmsg); } } else { // RDH und PinTan (= 2 und 999) try { // falls noch keine system-id ausgehandelt wurde, so sendet der //
         * hbci-server auch keine... deshalb der try-catch-block u_sysid=msg.getValueOfDE(sigheadName+".SecIdnDetails.sysid"); } catch (Exception e) {
         * u_sysid="0"; } }
         */
        ...
        /*
         * if (!u_secfunc.equals(mainPassport.getSigFunction())) { String errmsg=HBCIUtilsInternal.getLocMsg("EXCMSG_SIGTYPEFAIL",new String[]
         * {u_secfunc,mainPassport.getSigFunction()}); if (!HBCIUtilsInternal.ignoreError(null,"client.errors.ignoreSignErrors",errmsg)) throw new
         * HBCI_Exception(errmsg); }
         */
        ...
        /*
         * if (!u_sigalg.equals(passport.getSigAlg())) throw new HBCI_Exception(HBCIUtils.getLocMsg("EXCMSG_SIGALGFAIL",new String[]
         * {u_sigalg,passport.getSigAlg()})); if (!u_sigmode.equals(passport.getSigMode())) throw new
         * HBCI_Exception(HBCIUtils.getLocMsg("EXCMSG_SIGMODEFAIL",new String[] {u_sigmode,passport.getSigMode()})); if
         * (!u_hashalg.equals(passport.getHashAlg())) throw new HBCI_Exception(HBCIUtils.getLocMsg("EXCMSG_SIGHASHFAIL",new String[]
        ...
        if (mainPassport.hasInstSigKey()) {
            ...
            if (dontsignAttr.length() == 0) {
                if (hasSig()) {
                    ...
                    try {
                        /*
                         * first calculate hash-result, then verify the hashresult. In most cases, the hash() step will be executed by the signature algorithm,
                         * so the hash() call returns the message as-is. Currently the only exception is PKCS#1-10, where an extra round of hashing must be
                         * executed before applying the signature process
                         */
                    ...
                    /*
                     * das ist nur für den fall, dass das institut prinzipiell nicht signiert (also für den client-code); die verify()-funktion für den
                     * server-code überprüft selbstständig, ob tatsächlich eine benötigte signatur vorhanden ist (verlässt sich also nicht auf dieses TRUE, was
                     * beim fehlen einer signatur zurückgegeben wird
                     */
                    ret = true;
                }
            } else {
