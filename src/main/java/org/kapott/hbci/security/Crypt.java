...
public final class Crypt {
    ...
    private byte[] getPlainString() {
        try {
            ...
            /*
             * skip one segment at start and one segment at end of message (msghead and msgtail), the rest will be encrypted
             */
        ...
        if (passport.hasInstEncKey()) {
            ...
            if (dontcryptAttr.length() == 0) {
                try {
                    ...
                    newmsg.propagateValue(newmsg.getPath() + ".MsgTail.SegHead.seq", segnum, SyntaxElement.DONT_TRY_TO_CREATE, SyntaxElement.ALLOW_OVERWRITE);
                    newmsg.autoSetMsgSize(gen);
                } catch (Exception ex) {
                    throw new HBCI_Exception("*** error while encrypting", ex);
                }
            } else
                HBCIUtils.log("did not encrypt - message does not want to be encrypted", HBCIUtils.LOG_DEBUG);
        } else
            HBCIUtils.log("can not encrypt - no encryption key available", HBCIUtils.LOG_WARN);
        ...
        if (passport.hasMyEncKey()) {
            if (isCrypted()) {
                try {
                    ...
                    byte[] cryptedkey = crypthead.getValueOfDE(msgName + ".CryptHead.CryptAlg.enckey").getBytes(Comm.ENCODING);
                    ...
                    if (!secfunc.equals(passport.getCryptFunction())) {
                        String errmsg = HBCIUtilsInternal.getLocMsg("EXCMSG_CRYPTSFFAIL", new Object[] { secfunc, passport.getCryptFunction() });
                    ...
                    /*
                     * // constraint checking String keytype=crypthead.getValueOfDE(msgName+".CryptHead.CryptAlg.keytype"); if
                     * (!keytype.equals(passport.getSecMethod56()) && !(passport instanceof HBCIPassportPinTan)) throw new
                     * HBCI_Exception(HBCIUtils.getLocMsg("EXCMSG_CRYPTMETHODFAIL",new Object[] {keytype,passport.getSecMethod56()})); String
                     * mode=crypthead.getValueOfDE(msgName+".CryptHead.CryptAlg.mode"); if (!mode.equals(passport.getCryptMode())) throw new
                     * HBCI_Exception(HBCIUtils.getLocMsg("EXCMSG_CRYPTMODEFAIL",new Object[] {keytype,passport.getCryptMode()}));
                     */

                    /*
                     * TODO: removed code because no real checks are done here if (passport.getSysStatus().equals("1")) { String sysid=null; try { // falls noch
                     * keine system-id ausgehandelt wurde, so sendet der // hbci-server auch keine... deshalb der try-catch-block
                     * sysid=crypthead.getValueOfDE(msgName+".CryptHead.SecIdnDetails.sysid"); } catch (Exception e) { sysid="0"; }
                     * 
                     * // TODO: sysid checken (kann eigentlich auch entfallen, weil // das jeweils auf höherer ebene geschehen sollte!) } else { String
                     * cid=crypthead.getValueOfDE(msgName+".CryptHead.SecIdnDetails.cid"); if (!cid.equals(passport.getCID())) { String
                     * errmsg=HBCIUtilsInternal.getLocMsg("EXCMSG_CRYPTCIDFAIL"); if
                     * (!HBCIUtilsInternal.ignoreError(null,"client.errors.ignoreCryptErrors",errmsg)) throw new HBCI_Exception(errmsg); }
                     * 
                     * // TODO: cid checken }
                     */
                    ...
                    ret.append(msghead.toString(0)).append(new String(plainMsg, 0, plainMsg.length - padLength, Comm.ENCODING)).append(msgtail.toString(0));
                ...
                HBCIUtils.log("did not decrypt - message is already cleartext", HBCIUtils.LOG_DEBUG);
        } else
            HBCIUtils.log("can not decrypt - no decryption key available", HBCIUtils.LOG_WARN);
