...
public class RSABankData {
    ...
    private void fillRecord(byte[] record, int offset, int length, String value) {
        ...
        if (bytes.length > length)
            throw new HBCI_Exception("string value for bank data record at offset " + offset + " is " + bytes.length
                            + " bytes long but must not be longer than " + length + " bytes");
    ...
    public String toString() {
        return "index=" + index + " country=" + country + " bankCode=" + bankCode + " userId=" + userId + " comService=" + comService + " comAddress="
                        + comAddress + " comSuffix=" + comSuffix + " bankId=" + bankId + " systemId=" + systemId + " customerId=" + customerId;
    }
