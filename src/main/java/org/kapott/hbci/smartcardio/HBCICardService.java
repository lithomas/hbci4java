...
/**
 * HBCI-Cardservice fuer den DDVPCSC-Passport, basierend auf dem OCF-Code aus HBCI4Java 2.5.8.
 */
public abstract class HBCICardService {
    ...
    private final static String[] FEATURES = new String[] { "NO_FEATURE", "FEATURE_VERIFY_PIN_START", "FEATURE_VERIFY_PIN_FINISH", "FEATURE_MODIFY_PIN_START",
                    "FEATURE_MODIFY_PIN_FINISH", "FEATURE_GET_KEY_PRESSED", "FEATURE_VERIFY_PIN_DIRECT", "FEATURE_MODIFY_PIN_DIRECT",
                    "FEATURE_MCT_READER_DIRECT", "FEATURE_MCT_UNIVERSAL", "FEATURE_IFD_PIN_PROPERTIES", "FEATURE_ABORT", "FEATURE_SET_SPE_MESSAGE",
                    "FEATURE_VERIFY_PIN_DIRECT_APP_ID", "FEATURE_MODIFY_PIN_DIRECT_APP_ID", "FEATURE_WRITE_DISPLAY", "FEATURE_GET_KEY",
                    "FEATURE_IFD_DISPLAY_PROPERTIES", "FEATURE_GET_TLV_PROPERTIES", // NEU
    ...
    public void init(Card card) {
        ...
        try {
            ...
            for (int i = 0; i < response.length; i += 6) {
                ...
                Integer ioctl = new Integer((0xff & response[i + 2]) << 24) | ((0xff & response[i + 3]) << 16) | ((0xff & response[i + 4]) << 8)
                                | (0xff & response[i + 5]);
    ...
    public void verifySoftPIN(int pwdId, byte[] softPin) {
        byte[] body = new byte[] { (byte) 0x25, (byte) 0xff, (byte) 0xff, (byte) 0xff, (byte) 0xff, (byte) 0xff, (byte) 0xff, (byte) 0xff };
        ...
        CommandAPDU command = new CommandAPDU(SECCOS_CLA_STD, SECCOS_INS_VERIFY, (byte) 0x00, (byte) (SECCOS_PWD_TYPE_DF | pwdId), body);
        send(command);
    }

    protected void writeRecordBySFI(int sfi, int idx, byte[] data) {
        CommandAPDU command = new CommandAPDU(SECCOS_CLA_STD, SECCOS_INS_WRITE_RECORD, (byte) (idx + 1), (byte) ((sfi << 3) | 0x04), data);
        send(command);
    }

    protected void updateRecordBySFI(int sfi, int idx, byte[] data) {
        CommandAPDU command = new CommandAPDU(SECCOS_CLA_STD, SECCOS_INS_UPDATE_RECORD, (idx + 1), ((sfi << 3) | 0x04), data);
        send(command);
    }

    protected byte[] readRecordBySFI(int sfi, int idx) {
        CommandAPDU command = new CommandAPDU(SECCOS_CLA_STD, SECCOS_INS_READ_RECORD, (idx + 1), ((sfi << 3) | 0x04), 256);
    ...
    protected void selectSubFile(int id) {
        CommandAPDU command = new CommandAPDU(SECCOS_CLA_STD, SECCOS_INS_SELECT_FILE, 0x02, SECCOS_SELECT_RET_NOTHING, new byte[] { (byte) ((id >> 8) & 0xFF),
                        (byte) (id & 0xFF) });
        send(command);
    }

    protected byte[] getKeyInfo(int idx) {
        CommandAPDU command = new CommandAPDU(SECCOS_CLA_EXT, SECCOS_INS_GET_KEYINFO, SECCOS_KEY_TYPE_DF, (idx + 1), 256);
        return receive(command);
    }

    protected void putData(int tag, byte[] data) {
        CommandAPDU command = new CommandAPDU(SECCOS_CLA_STD, SECCOS_INS_PUT_DATA, (byte) ((tag >> 8) & 0xFF), (byte) (tag & 0xFF), data);
        send(command);
    }

    protected byte[] getChallenge() {
        CommandAPDU command = new CommandAPDU(SECCOS_CLA_STD, SECCOS_INS_GET_CHALLENGE, 0x00, 0x00, 8);
        return receive(command);
    }

    protected byte[] internalAuthenticate(int keynum, byte[] challenge) {
        CommandAPDU command = new CommandAPDU(SECCOS_CLA_STD, SECCOS_INS_INT_AUTH, 0x00, (SECCOS_KEY_TYPE_DF | keynum), challenge, 8);
        return receive(command);
    }
    ...
    protected byte[] createPINVerificationDataStructure(int pwdId) throws IOException {
        ...
        byte[] verifyApdu = new byte[] { SECCOS_CLA_STD, // CLA
