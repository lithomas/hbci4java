...
public class RSACardService extends HBCICardService {
    ...
    public void init(Card card) {
        ...
        ATR atr = card.getATR();
        if (!Arrays.equals(atr.getBytes(), new byte[] { (byte) 0x3b, (byte) 0xb7, (byte) 0x94, (byte) 0x00, (byte) 0x81, (byte) 0x31, (byte) 0xfe, (byte) 0x65,
                        (byte) 0x53, (byte) 0x50, (byte) 0x4b, (byte) 0x32, (byte) 0x33, (byte) 0x90, (byte) 0x00, (byte) 0xd1 })) {
    ...
    protected byte[] createPINVerificationDataStructure(int pwdId) throws IOException {
        ...
        byte[] verifyApdu = new byte[] { ISO7816_CLA_STD, // CLA
    ...
    public void verifySoftPIN(int pwdId, byte[] softPin) {
        ...
        byte[] body = new byte[] { (byte) 0x20, (byte) 0x20, (byte) 0x20, (byte) 0x20, (byte) 0x20, (byte) 0x20, (byte) 0x20, (byte) 0x20 };

        System.arraycopy(softPin, 0, body, 0, softPin.length);

        CommandAPDU command = new CommandAPDU(ISO7816_CLA_STD, ISO7816_INS_VERIFY, (byte) 0x00, (byte) (ISO7816_PWD_TYPE_DF | pwdId), body);
        send(command);
    }
    ...
    public RSAKeyData[] readKeyData(int idx) {
        ...
        if (rawData == null)
            return null;
        return new RSAKeyData[] { new RSAKeyData(idx, RSAKeyData.Type.VERIFY, rawData, verifyPKData),
                        new RSAKeyData(idx, RSAKeyData.Type.ENCIPHER, rawData, encipherPKData), new RSAKeyData(idx, RSAKeyData.Type.SIGN, rawData, signPKData),
    ...
    public byte[] sign(int idx, byte[] data) {
        ...
        send(new CommandAPDU(ISO7816_CLA_STD, 0x22, 0x41, 0xB6 /* signature */, new byte[] { (byte) 0x84, // private key
                        (byte) 0x01, // length
                        (byte) (0x81 + idx), // kid
    ...
    public byte[] encipher(int idx, byte[] data) {
        ...
        send(new CommandAPDU(ISO7816_CLA_STD, 0x22, 0x41, 0xB8 /* cipher */, new byte[] { (byte) 0x84, // private key
                        (byte) 0x01, // length
                        (byte) (0x86 + idx), // kid
                        (byte) 0x83, // public key
                        (byte) 0x01, // length
