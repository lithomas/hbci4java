...
public abstract class AbstractSEPAGV extends HBCIJobImpl {
    /**
     * Token, der als End-to-End ID Platzhalter verwendet wird, wenn keine angegeben wurde. In pain.001.001.02 wurde dieser Token noch explizit erwaehnt.
     * Inzwischen nicht mehr. Nach Ruecksprache mit Holger vom onlinebanking-forum.de weiss ich aber, dass VRNetworld den auch verwendet und er von Banken als
    ...
    /**
     * Durchsucht das BPD-Segment "HISPAS" nach dem Property "cannationalacc" um herauszufinden, ob beim Versand eines SEPA-Auftrages die nationale
     * Bankverbindung angegeben sein darf.
     * 
     * Siehe FinTS_3.0_Messages_Geschaeftsvorfaelle_2013-05-28_final_version.pdf - Kapitel B.3.2
     * 
     * @param handler
     * @return true, wenn der BPD-Parameter von der Bank mit "J" befuellt ist und die nationale Bankverbindung angegeben sein darf.
     */
    ...
    /**
     * Diese Methode schaut in den BPD nach den unterstützen pain Versionen (bei LastSEPA pain.008.xxx.xx) und vergleicht diese mit den von HBCI4Java
     * unterstützen pain Versionen. Der größte gemeinsamme Nenner wird zurueckgeliefert.
     * 
     * @param handler
     * @param gvName
     *            der Geschaeftsvorfall fuer den in den BPD nach dem PAIN-Versionen gesucht werden soll.
    ...
    /**
     * @see org.kapott.hbci.GV.HBCIJobImpl#setLowlevelParam(java.lang.String, java.lang.String) This is needed to "redirect" the sepa values. They dont have to
     *      stored directly in the message, but have to go into the SEPA document which will by created later (in verifyConstraints())
     */
    ...
    /**
     * This is needed for verifyConstraints(). Because verifyConstraints() tries to read the lowlevel-values for each constraint, the lowlevel-values for
     * sepa.xxx would always be empty (because they do not exist in hbci messages). So we read the sepa lowlevel-values from the special sepa structure instead
    ...
    /**
     * @see org.kapott.hbci.GV.HBCIJobImpl#addConstraint(java.lang.String, java.lang.String, java.lang.String, int, boolean) Ueberschrieben, um die
     *      Default-Werte der SEPA-Parameter vorher rauszufischen und in "this.sepaParams" zu speichern. Die brauchen wir "createSEPAFromParams" beim Erstellen
     *      des XML - sie wuerden dort sonst aber fehlen, weil Default-Werte eigentlich erst in "verifyConstraints" uebernommen werden.
     */
    @Override
